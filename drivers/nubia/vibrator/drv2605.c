...
//add debug by wxf
//#define DRV26XX_DEBUG
#ifdef DRV26XX_DEBUG
	#define drv26xx_debug(format , ...)   printk(format, ##__VA_ARGS__)
#else
...
static ssize_t dev2605_read(struct file* filp, char* buff, size_t length, loff_t* offset)
{
	...
	if(pDrv2605data->ReadLen > 0){
		...
		if (ret != 0){
			drv26xx_debug("%s, copy_to_user err=%d \n", __FUNCTION__, ret);
		}else{
	...
	switch(buff[0])
	{
			...
			if(((len-1)%2) == 0){
				...
				if((pDrv2605data->RTPSeq.RTPCounts <= MAX_RTP_SEQ)&&(pDrv2605data->RTPSeq.RTPCounts>0)){
					if(copy_from_user(pDrv2605data->RTPSeq.RTPData, &buff[1], pDrv2605data->RTPSeq.RTPCounts*2) != 0){
						drv26xx_debug("%s, rtp_seq copy seq err\n", __FUNCTION__);
						break;
					}
					...
					drv26xx_debug("%s, rtp_seq count error,maximum=%d\n", __FUNCTION__,MAX_RTP_SEQ);
				}
			}else{
				drv26xx_debug("%s, rtp_seq len error\n", __FUNCTION__);
			}
			break;
		}
		...
		case HAPTIC_CMDID_REG_READ:
		{
			if(len == 2){
				...
				drv26xx_debug("%s, reg_read len error\n", __FUNCTION__);
			}
			break;
		}
		...
		case HAPTIC_CMDID_REG_WRITE:
		{
			if((len-1) == 2){
				...
				drv26xx_debug("%s, reg_write len error\n", __FUNCTION__);
			}
			break;
		}
		...
		default:
			drv26xx_debug("%s, unknown HAPTIC cmd\n", __FUNCTION__);
		break;
	}
...
static int Haptics_init(struct drv2605_data *pDrv2605data)
{
	...
	if (reval < 0)
	{
		drv26xx_debug(KERN_ALERT"drv2605: error getting major number %d\n", reval);
		goto fail0;
	}
	...
	{
		drv26xx_debug(KERN_ALERT"drv2605: error creating class\n");
		goto fail1;
	}
	...
	{
		drv26xx_debug(KERN_ALERT"drv2605: error creating device 2605\n");
		goto fail2;
	}
	...
	if (reval)
	{
		drv26xx_debug(KERN_ALERT"drv2605: fail to add cdev\n");
		goto fail3;
	}
...
static void dev_init_platform_data(struct drv2605_data *pDrv2605data)
{
	...
	if(pDrv2605data->OTP == 0) {
		...
		drv26xx_debug("%s, OTP programmed\n", __FUNCTION__);
	}
...
static int dev_auto_calibrate(struct drv2605_data *pDrv2605data)
{
	...
	drv26xx_debug("%s, calibration status =0x%x\n", __FUNCTION__, status);
...
//	struct drv2605_platform_data *pDrv2605Platdata = client->dev.platform_data;
	...
	drv2605_plat_data. GpioEnable= of_get_named_gpio(of_node, "ti,enable-gpio", 0);
	if (drv2605_plat_data. GpioEnable < 0) {
		drv26xx_debug("[drv2605]%s: Can't get drv2605-enable-gpio\n",__func__);
		return -EINVAL;
	}
	drv26xx_debug("drv2605 gpio enable :%d\n",drv2605_plat_data. GpioEnable);

	if (!i2c_check_functionality(client->adapter, I2C_FUNC_I2C))
	{
		drv26xx_debug(KERN_ERR"%s:I2C check failed\n", __FUNCTION__);
		return -ENODEV;
	}

	pDrv2605data = devm_kzalloc(&client->dev, sizeof(struct drv2605_data), GFP_KERNEL);
	if (pDrv2605data == NULL){
		drv26xx_debug(KERN_ERR"%s:no memory\n", __FUNCTION__);
		return -ENOMEM;
	}
	...
	if (IS_ERR(pDrv2605data->regmap)) {
		...
		drv26xx_debug(KERN_ERR"%s:Failed to allocate register map: %d\n",__FUNCTION__,err);
		return err;
	}
	...
	if(pDrv2605data->PlatData.GpioTrigger){
		...
		if(err < 0){
			drv26xx_debug(KERN_ERR"%s: GPIO request Trigger error\n", __FUNCTION__);
	...
	if(pDrv2605data->PlatData.GpioEnable){
		...
		if(err < 0){
			drv26xx_debug(KERN_ERR"%s: GPIO request enable error\n", __FUNCTION__);
			goto exit_gpio_request_failed;
		}
	...
	if(err < 0){
		drv26xx_debug("%s, i2c bus fail (%d)\n", __FUNCTION__, err);
		goto exit_gpio_request_failed;
	}else{
		drv26xx_debug("%s, i2c status (0x%x)\n", __FUNCTION__, err);
		status = err;
	}
	/* Read device ID */
	...
	switch (pDrv2605data->device_id)
	{
		...
		drv26xx_debug("drv2605 driver found: drv2605 v1.1.\n");
		break;
		case DRV2605_VER_1DOT0:
		drv26xx_debug("drv2605 driver found: drv2605 v1.0.\n");
		break;
		case DRV2604:
		drv26xx_debug(KERN_ALERT"drv2605 driver found: drv2604.\n");
		break;
		default:
		drv26xx_debug(KERN_ERR"drv2605 driver found: unknown.\n");
		break;
	}

	if((pDrv2605data->device_id != DRV2605_VER_1DOT1)
		&&(pDrv2605data->device_id != DRV2605_VER_1DOT0)){
		drv26xx_debug("%s, status(0x%x),device_id(%d) fail\n",
	...
	if(pDrv2605data->OTP == 0){
		...
		if(err < 0){
			drv26xx_debug("%s, ERROR, calibration fail\n",	__FUNCTION__);
		}
	}
	#endif
	...
	drv26xx_debug("drv2605 probe succeeded\n");
...
exit_gpio_request_failed:
	...
	drv26xx_debug(KERN_ERR"%s failed, err=%d\n",__FUNCTION__, err);
	return err;
}
...
static int drv2605_remove(struct i2c_client* client)
{
	...
	drv26xx_debug(KERN_ALERT"drv2605 remove");
