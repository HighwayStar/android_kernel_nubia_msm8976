...
struct fusb301_info fusb301_typec_info;

#define TTRYTO_EXP_TIME	600
#define TCCDEBOUNCEMAX_TIME	200
static int fusb301_poweroff(struct notifier_block *nb, unsigned long event, void *unused);
static struct notifier_block fusb301_poweroff_notifier = {
        .notifier_call = fusb301_poweroff,
};
...
static int fusb301_poweroff(struct notifier_block *nb, unsigned long event, void *unused)
{
    switch (event) {
        ...
        case SYS_HALT:
            ...
            break;
        case SYS_POWER_OFF:
            USB_LOG_ERROR("SYS_POWER_OFF\n");
			if (fusb301_typec_info.i2c == NULL)
			{
			    USB_LOG_ERROR("I2C IS NULL\n");
			    break;
			}
            (void)fusb301_write_reg(fusb301_typec_info.i2c, REG_MAN, MAN_DIS);
            break;
        default:
            break;
        }
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if(data == FUSB301_MODE_SRC)
		...
		USB_LOG_INFO("Argument is not match!!\n");

	fusb301_read_reg(info->i2c, REG_MOD, &rdata);
	USB_LOG_INFO("REG_MOD(0x%02x)\n",rdata);

	return count;
}

static DEVICE_ATTR(type, 0444, show_type, NULL);
static DEVICE_ATTR(mode, 0222, NULL, store_mode);
...
static void fusb301_initialization(struct fusb301_info *info)
{
	...
	fusb301_update_reg(info->i2c, REG_MAN, 0, MAN_DIS);  //clear disable bit
	fusb301_write_reg(info->i2c, REG_MOD, MOD_DRP_ACC);
	info->state = FUSB301_UNATTACHED_DRP;
	info->TriedSink = 0;
	...
	struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	register_reboot_notifier(&fusb301_poweroff_notifier);
	...
	ret = device_create_file(info->dev_t, &dev_attr_mode);
	if (ret < 0) {
		USB_LOG_ERROR("device_create_file  failed\n");
		goto create_dev_failed;
	}
...
*/
	...
	unregister_reboot_notifier(&fusb301_poweroff_notifier);
	kfree(info);
