...
static int pi5u_read_reg_all(struct i2c_client *i2c, BYTE *dest)
{
	int ret;

	ret = i2c_master_recv(i2c, dest, 4);
	if (ret < 0) {
		USB_LOG_ERROR("failed to read register, ret(%d)\n", ret);
		return ret;
	}
	return 0;
}
	...
	switch (reg) {
		...
		case REG_CC_STAT:
			*dest = buf[3];
			break;
		default:
			USB_LOG_ERROR("read wrong register 0x%x\n", reg);
			return -1;
	}
	return 0;
}

static int pi5u_write_reg(struct i2c_client *i2c, BYTE data)
{
	int ret;
	BYTE buf[2] = {0};
	...
	if (ret < 0) {
		USB_LOG_ERROR("failed to write register with data 0x%x, ret(%d)\n",data, ret);
		return ret;
	}
	return 0;
}
...
static int pericom_typec_reset(struct i2c_client *client,u8 chipid)
{
	...
	ret = pi5u_write_reg(client, 0x01);
	if(ret < 0)
	{
	...
	ret = pi5u_write_reg(client, 0x04);
	if(ret < 0)
	{
		USB_LOG_ERROR("Write i2c block data error !");
		goto error;
	...
	ret = pi5u_read_reg_all(client, regbuf);
	if(ret < 0)
	{
...
/*******************************NUBIA END*********************************/
	...
	if (value >= 0)
		...
		return -ENODEV;
	value = of_get_named_gpio_flags(np, "pericom,cdc-gpio", 0, NULL);
	if (value >= 0)
		pericom_typec_gpio[CC_CDC_HS_DET_INDEX].gpio = value;
	else
		return -ENODEV;
	value = of_get_named_gpio_flags(np, "pericom,audio-gpio", 0, NULL);
	if (value >= 0)
		pericom_typec_gpio[CC_AUDIO_SWITCH_INDEX].gpio = value;
	else
		return -ENODEV;
	value = of_get_named_gpio_flags(np, "pericom,switch-en-gpio", 0, NULL);
	if (value >= 0)
		pericom_typec_gpio[CC_SWITCH_EN_INDEX].gpio = value;
	else
		return -ENODEV;

	USB_LOG_INFO("Interrupt GPIO = %u\n",pericom_typec_gpio[INT_INDEX].gpio);
	USB_LOG_INFO("CC_ENABLE GPIO = %u\n", pericom_typec_gpio[CC_EN].gpio);
	USB_LOG_INFO("CC_CDC_HS_DET_INDEX GPIO = %u\n",pericom_typec_gpio[CC_CDC_HS_DET_INDEX].gpio);
	USB_LOG_INFO("CC_AUDIO_SWITCH_INDEX GPIO = %u\n", pericom_typec_gpio[CC_AUDIO_SWITCH_INDEX].gpio);
	USB_LOG_INFO("CC_SWITCH_EN_INDEX = %u\n",pericom_typec_gpio[CC_SWITCH_EN_INDEX].gpio);
	return 0;
}
...
static int pi5u_switch_audio_mode(struct pi5u_info *info, int mode)
{
	int ret;

	switch (mode) {
		case 0:
			ret = gpio_direction_output(pericom_typec_gpio[CC_CDC_HS_DET_INDEX].gpio, 1);
			if(ret != 0)
			{
				USB_LOG_ERROR("set CC_CDC_HS_DET_INDEX init failed!\n");
				return ret;
			}
			ret = gpio_direction_output(pericom_typec_gpio[CC_AUDIO_SWITCH_INDEX].gpio, 0);
			if(ret != 0)
			{
				USB_LOG_ERROR("set CC_AUDIO_SWITCH_INDEX init failed!\n");
				return ret;
			}
			break;
		case 1:
			ret = gpio_direction_output(pericom_typec_gpio[CC_CDC_HS_DET_INDEX].gpio, 0);
			if(ret != 0)
			{
				USB_LOG_ERROR("set CC_CDC_HS_DET_INDEX init failed!\n");
				return ret;
			}
			ret = gpio_direction_output(pericom_typec_gpio[CC_AUDIO_SWITCH_INDEX].gpio, 1);
			if(ret != 0)
			{
				USB_LOG_ERROR("set CC_AUDIO_SWITCH_INDEX init failed!\n");
				return ret;
...
irqreturn_t pericom_30216A_irq_thread_handler(int irq, void *dev_id)
{
	...
	int ret = 0;
	/*1.mask interrupt*/
	ret = pi5u_write_reg(info->i2c, 0x05);
	if(ret < 0)
	{
	...
	msleep(20);
	/*3. read byte 1-4*/
	ret = pi5u_read_reg_all(info->i2c, regbuf);
	if(ret < 0)
	{
		USB_LOG_ERROR("Read i2c byte 1-4 failed!");
		goto exit_unmask_int;
	}
	USB_LOG_INFO("Interrupt: 0x%x,0x%x,0x%x,0x%x\n",regbuf[0],regbuf[1],regbuf[2],regbuf[3]);
	/*4.proccessing*/
	if(regbuf[3] == PERICOM_TYPEC_ERROR_STATE)
	{
		USB_LOG_ERROR("TypeC state error!");
		ret = pi5u_write_reg(info->i2c, 0x01);
		if(ret < 0)
		{
		USB_LOG_ERROR("Write i2c block data error !");
		goto exit_unmask_int;
...
exit_unmask_int:
	/*5.unmask interrupt*/
	msleep(20);
	ret = pi5u_write_reg(info->i2c, 0x04);
	if(ret < 0)
	{
		USB_LOG_ERROR("Write i2c block data error !");
	}
...
irqreturn_t pericom_30216D_irq_thread_handler(int irq, void *dev_id)
{
	struct pi5u_info *info =  dev_id;
	u8 regbuf[4] = { 0 };
	int ret = 0;
	...
	u8 int_status = 0;
	u8 cc_status = 0;
	enum pi5u_port_status port_status = PI5U_PORT_STATUS_STANDBY;
	...
	ctrl_status = regbuf[1];
	int_status = regbuf[2];
	cc_status = regbuf[3];
	port_status = (cc_status >> 2) & 0x07;

	switch(port_status)
	{
		case PI5U_PORT_STATUS_STANDBY:
			USB_LOG_INFO("Port standby\n");
			pi5u_switch_audio_mode(info, 0);
			break;
		case PI5U_PORT_STATUS_DEVICE:
			USB_LOG_INFO("Device plug in\n");
			pi5u_switch_audio_mode(info, 0);
			break;
		case PI5U_PORT_STATUS_HOST:
			USB_LOG_INFO("Host plug in\n");
			pi5u_switch_audio_mode(info, 0);
			break;
		case PI5U_PORT_STATUS_DEBUG_ACCESSORY:
			USB_LOG_INFO("Debug accessory plug in\n");
			pi5u_switch_audio_mode(info, 0);
			break;
		case PI5U_PORT_STATUS_DEVICE_WITH_ACTIVE_TABLE:
			USB_LOG_INFO("Device plug in with active cable\n");
			pi5u_switch_audio_mode(info, 0);
			break;
		case PI5U_PORT_STATUS_AUDIO_ACCESSORY:
			USB_LOG_INFO("Audio accessory plug in\n");
			pi5u_switch_audio_mode(info, 1);
			break;
		default:
			USB_LOG_ERROR("PI5U_PORT_STATUS_UNKNOWN\n");
			pi5u_switch_audio_mode(info, 0);
	}
...
exit_unmask_int:
	...
	if(ret < 0)
	{
		USB_LOG_ERROR("Unmask interrupt failed!\n");
	}
	return IRQ_HANDLED;
}
	...
	struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	USB_LOG_ERROR("set CC_CDC_HS_DET_INDEX init success!\n");
	...
	USB_LOG_ERROR("set CC_AUDIO_SWITCH_INDEX init success!\n");

	ret = gpio_direction_output(pericom_typec_gpio[CC_SWITCH_EN_INDEX].gpio,0);
	if(ret != 0){
		USB_LOG_ERROR("set CC_SWITCH_EN_INDEX init failed!\n");
		goto exit_gpio_request_failed;
	}
	USB_LOG_ERROR("set CC_SWITCH_EN_INDEX init success!\n");
	...
	switch (chip_id) {
		case PI5U_30216A_ID:
			...
			break;
		case PI5U_30216D_ID:
			ret = request_threaded_irq(info->irq,NULL,pericom_30216D_irq_thread_handler,
				IRQF_ONESHOT | IRQF_TRIGGER_LOW, LOG_TAG, info);
			if(ret < 0){
				USB_LOG_ERROR("request IRQ failed!\n");
				goto exit_read_id_failed;
