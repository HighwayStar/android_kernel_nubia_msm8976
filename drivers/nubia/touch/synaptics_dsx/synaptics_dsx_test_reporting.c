...
//test_sysfs_resume_touch_store
	...
	case F54_ABS_DELTA_CAP:
		break;
	default:
#ifdef NUBIA_SYNAPTICS_IC_DETECT_RESET_CHECK
		rmi4_data->resetting = true;
		rmi4_data->reset_device(rmi4_data, false);
		rmi4_data->resetting = false;

		if (rmi4_data->reinit){
			pr_err("%s:reinit is needed after reset, line = %d\n",
					__func__, __LINE__);
			rmi4_data->reinit_device(rmi4_data);
			rmi4_data->reinit = false;
			rmi4_data->resetting = true;
			rmi4_data->reset_device(rmi4_data, false);
			rmi4_data->resetting = false;
		}
		...
		struct device_attribute *attr, char *buf)
{
	...
	short up_cap = 3000; //max value, this will be reconfiged later
...
#ifndef NUBIA_TP_DETECT_OLD
        int rx_vk = 3;
        int tx_vk = 1;

        unsigned char always_log = /* 1 */ 0;
        unsigned char test_key = 1;/* for nx563, we donnot check pid, default to enable check key */
...
#define PBUF_LEN 1024
        ...
        //key leveel
        short key_1[2] = {5500, 7000};
        short key_2[2] = {5500, 7000};
        short key_3[2] = {5500, 7000};

        struct synaptics_rmi4_data *rmi4_data = f54->rmi4_data;
        unsigned char *productid = rmi4_data->rmi4_mod_info.product_id_string;

        if (!strncmp(productid, "NUBIA-CTCT", 10))
        {
            rx_vk = 3;
            tx_vk = 1;
            low_cap = 1000;
            up_cap = 3000;
            test_key = 1;
            key_1[0] = 1500; key_1[1] = 3500;
            key_2[0] = 3000; key_2[1] = 5000;
            key_3[0] = 1500; key_3[1] = 3500;
        }
        else if ( !strncmp(productid, "s3320", 10) ||  !strncmp(productid, "NX569-JDIA", 10) )
        {
            rx_vk = 3;
            tx_vk = 1;
            low_cap = 1000;
...
step1_done:
       ...
       {
            //Oppos, panel may be different...
            //donot check for 563
            //if (!strncmp(productid, "NUBIA-CTCT", 10) || !strncmp(productid, "s3320", 10) ||  !strncmp(productid, "NX569-JDIA", 10) )
            {
                ...
                if (  \
                    ...
                    (report_data_16[ ( tx_num -1 ) *  rx_num + rx_num - 1 ] < key_3[0] || report_data_16[ ( tx_num -1 ) *  rx_num + rx_num - 1 ] > key_3[1] )   \
                        ...
                        dev_err(rmi4_data->pdev->dev.parent, "%s: test fail, key1=%d, key2=%d, key3=%d\n", \
                            __func__, \
...
static int test_set_sysfs(void)
{
	...
	retval = sysfs_create_group(&rmi4_data->pdev->dev.parent->kobj, &attr_group);
	if (retval < 0) {
		dev_err(rmi4_data->pdev->dev.parent,
				"%s: Failed to create sysfs attributes\n",
				__func__);
