...
#define SYNAPTICS_PALM_GESTURE_REPORT 0x0600
...
//nubia add for tp reset after incall begin
static ssize_t synaptics_rmi4_reset_show(struct device *dev,
		struct device_attribute *attr, char *buf);
...
#ifdef NUBIA_SYNAPTICS_RESUME_WORK_QUEUE
static void synaptics_resume_work_func(struct work_struct *work);
#endif

#ifdef NUBIA_ENABLE_WAKEUP_GESTURE_FLAG
...
static struct synaptics_rmi4_data *nubia_i2c_node;
#endif
...
static bool synaptics_f12_palm_touch(struct synaptics_rmi4_data *rmi4_data)
{
	int retval;
	char detected_palm = 0;
	retval = synaptics_rmi4_reg_read(rmi4_data,SYNAPTICS_PALM_GESTURE_REPORT,
			&detected_palm,
			sizeof(detected_palm));
	return ((detected_palm&1) == 1) ? true : false;
}
		...
		bool report)
{
...
#ifdef NUBIA_ENABLE_WAKEUP_GESTURE_FLAG
	//data[1] is interrupt status register
	if (rmi4_data->suspend && !rmi4_data->resume
		&& nubia_wakeup_gesture && ((data[1] & 0x02) == 0x02)) {
		pr_err("[TP]synaptics firmware unexpectedly reset, "
				"set wakeup gesture flag enable !\n");
	...
	if (status.unconfigured && !status.flash_prog) {
		...
		rmi4_data->reinit = true;
		pr_err("%s:reinit is needed !\n", __func__);
		if (rmi4_data->resetting) {
			pr_err("%s:doing resetting, postpone reinit !\n", __func__);
			return;
		}
#endif
...
#ifdef NUBIA_SYNAPTICS_IC_DETECT_RESET_CHECK
	rmi4_data->reinit_device = synaptics_rmi4_reinit_device;
#endif
...
#ifdef NUBIA_SYNAPTICS_RESUME_WORK_QUEUE
	rmi4_data->synaptics_resume_wq=
			create_singlethread_workqueue("synaptics_resume_workqueue");
	INIT_WORK(&rmi4_data->synaptics_resume_work, synaptics_resume_work_func);
#endif
...
#ifdef NUBIA_SYNAPTICS_RESUME_WORK_QUEUE
	cancel_work_sync(&rmi4_data->synaptics_resume_work);
	flush_workqueue(rmi4_data->synaptics_resume_wq);
	destroy_workqueue(rmi4_data->synaptics_resume_wq);
#endif
...
static int synaptics_rmi4_suspend(struct device *dev)
{
...
#ifdef NUBIA_SYNAPTICS_RESUME_WORK_QUEUE
	flush_work(&rmi4_data->synaptics_resume_work);
#endif
	...
	if (!rmi4_data->suspend) {
		...
		if (nubia_wakeup_gesture) {
			msleep(40);
			if(tp_splash_enable != NULL)
				pr_err("caiwutang *tp_splash_enable = %d\n",*tp_splash_enable);
			if (rmi4_data->hw_if->board_data->lcd_reset_gpio >= 0
				&& tp_splash_enable != NULL && !(*tp_splash_enable)) {
				ret = synaptics_rmi4_gpio_setup(
						rmi4_data->hw_if->board_data->lcd_reset_gpio,
						false, 0, 0);
				if (ret < 0) {
					dev_err(rmi4_data->pdev->dev.parent,
							"[TP]%s: failed to set lcd_reset_gpio low!\n",
							__func__);
				}
			}
		} else {
...
static int synaptics_rmi4_resume(struct device *dev)
{
	...
	if (rmi4_data->suspend && rmi4_data->enable_wakeup_gesture) {
#ifdef NUBIA_SYNAPTICS_RESUME_WORK_QUEUE
		queue_work(rmi4_data->synaptics_resume_wq,
				&rmi4_data->synaptics_resume_work);
#else
		synaptics_rmi4_wakeup_gesture(rmi4_data, false);
		disable_irq_wake(rmi4_data->irq);
		ret = synaptics_rmi4_reset_device(rmi4_data, false);
	...
	if(rmi4_data->suspend) {
		...
		if (nubia_wakeup_gesture) {
			...
			if (rmi4_data->hw_if->board_data->lcd_reset_gpio >= 0
				...
				if (ret < 0) {
					dev_err(rmi4_data->pdev->dev.parent,
							"[TP]%s: failed to set lcd_reset_gpio high!\n",
							__func__);
				}
			}
			msleep(10);
