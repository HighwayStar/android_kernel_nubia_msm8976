...
#define APEN_PHYS_NAME "btn_synaptics_/active_pen"
...
struct btn_synaptics_rmi4_apen_handle {
	bool apen_present;
...
static struct btn_synaptics_rmi4_apen_handle *apen;
...
static void apen_report(void)
{
	...
	struct btn_synaptics_rmi4_data *rmi4_data = apen->rmi4_data;
...
static int apen_pressure(struct btn_synaptics_rmi4_f12_query_8 *query_8)
{
	int retval;
	unsigned char ii;
...
static void btn_synaptics_rmi4_apen_attn(struct btn_synaptics_rmi4_data *rmi4_data,
		unsigned char intr_mask)
{
	if (!apen)
		return;
...
static int btn_synaptics_rmi4_apen_init(struct btn_synaptics_rmi4_data *rmi4_data)
{
	int retval;
	...
	apen->apen_dev->id.product = BTN_SYNAPTICS_DRIVER_PRODUCT;
	apen->apen_dev->id.version = BTN_SYNAPTICS_DRIVER_VERSION;
...
static void btn_synaptics_rmi4_apen_remove(struct btn_synaptics_rmi4_data *rmi4_data)
{
	if (!apen)
		goto exit;
...
static void btn_synaptics_rmi4_apen_reset(struct btn_synaptics_rmi4_data *rmi4_data)
{
	if (!apen) {
		btn_synaptics_rmi4_apen_init(rmi4_data);
		return;
	}
...
static void btn_synaptics_rmi4_apen_reinit(struct btn_synaptics_rmi4_data *rmi4_data)
{
	if (!apen)
		return;
...
static void btn_synaptics_rmi4_apen_e_suspend(struct btn_synaptics_rmi4_data *rmi4_data)
{
	if (!apen)
		return;
...
static void btn_synaptics_rmi4_apen_suspend(struct btn_synaptics_rmi4_data *rmi4_data)
{
	if (!apen)
		return;
...
static struct btn_synaptics_rmi4_exp_fn active_pen_module = {
	.fn_type = RMI_ACTIVE_PEN,
	.init = btn_synaptics_rmi4_apen_init,
	.remove = btn_synaptics_rmi4_apen_remove,
	.reset = btn_synaptics_rmi4_apen_reset,
	.reinit = btn_synaptics_rmi4_apen_reinit,
	.early_suspend = btn_synaptics_rmi4_apen_e_suspend,
	.suspend = btn_synaptics_rmi4_apen_suspend,
	.resume = NULL,
	.late_resume = NULL,
	.attn = btn_synaptics_rmi4_apen_attn,
};

static int __init rmi4_active_pen_module_init(void)
{
	btn_synaptics_rmi4_new_function(&active_pen_module, true);
...
{
	btn_synaptics_rmi4_new_function(&active_pen_module, false);
...
MODULE_DESCRIPTION("BTN_Synaptics DSX Active Pen Module");
