...
struct btn_synaptics_rmi4_video_handle {
	unsigned char param;
...
static struct btn_synaptics_rmi4_video_handle *video;
...
static int video_send_dcs_command(unsigned char command_opcode)
{
	...
	struct btn_synaptics_rmi4_data *rmi4_data = video->rmi4_data;
...
static int video_scan_pdt(void)
{
	...
	for (page = 0; page < PAGES_TO_SERVICE; page++) {
		for (addr = PDT_START; addr > PDT_END; addr -= PDT_ENTRY_SIZE) {
			...
			if (rmi_fd.fn_number == BTN_SYNAPTICS_RMI4_F38) {
				f38_found = true;
				goto f38_found;
			}
		}
...
static int btn_synaptics_rmi4_video_init(struct btn_synaptics_rmi4_data *rmi4_data)
{
	int retval;
...
static void btn_synaptics_rmi4_video_remove(struct btn_synaptics_rmi4_data *rmi4_data)
{
	unsigned char attr_count;
...
static void btn_synaptics_rmi4_video_reset(struct btn_synaptics_rmi4_data *rmi4_data)
{
	if (!video)
		btn_synaptics_rmi4_video_init(rmi4_data);

	return;
}

#ifdef RMI_DCS_SUSPEND_RESUME
static void btn_synaptics_rmi4_video_suspend(struct btn_synaptics_rmi4_data *rmi4_data)
{
	int retval;
	unsigned char ii;
	unsigned char command;
...
static void btn_synaptics_rmi4_video_resume(struct btn_synaptics_rmi4_data *rmi4_data)
{
	int retval;
	unsigned char ii;
	unsigned char command;
...
static struct btn_synaptics_rmi4_exp_fn video_module = {
	.fn_type = RMI_VIDEO,
	.init = btn_synaptics_rmi4_video_init,
	.remove = btn_synaptics_rmi4_video_remove,
	.reset = btn_synaptics_rmi4_video_reset,
	.reinit = NULL,
	.early_suspend = NULL,
#ifdef RMI_DCS_SUSPEND_RESUME
	.suspend = btn_synaptics_rmi4_video_suspend,
	.resume = btn_synaptics_rmi4_video_resume,
#else
	.suspend = NULL,
	.resume = NULL,
#endif
...
{
	btn_synaptics_rmi4_new_function(&video_module, true);
...
{
	btn_synaptics_rmi4_new_function(&video_module, false);
...
MODULE_DESCRIPTION("BTN_Synaptics DSX Video Module");
