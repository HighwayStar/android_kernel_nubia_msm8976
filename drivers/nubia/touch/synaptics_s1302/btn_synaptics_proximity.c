...
#define PROX_PHYS_NAME "btn_synaptics_/proximity"

#define HOVER_Z_MAX (255)

#define HOVERING_FINGER_EN (1 << 4)

static ssize_t btn_synaptics_rmi4_hover_finger_en_show(struct device *dev,
		struct device_attribute *attr, char *buf);

static ssize_t btn_synaptics_rmi4_hover_finger_en_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count);

static struct device_attribute attrs[] = {
	__ATTR(hover_finger_en, (S_IRUGO | S_IWUGO),
			btn_synaptics_rmi4_hover_finger_en_show,
			btn_synaptics_rmi4_hover_finger_en_store),
};
...
struct btn_synaptics_rmi4_prox_handle {
...
static struct btn_synaptics_rmi4_prox_handle *prox;
...
static void prox_hover_finger_report(void)
{
	...
	struct btn_synaptics_rmi4_data *rmi4_data = prox->rmi4_data;
...
int btn_synaptics_rmi4_prox_hover_finger_en(bool enable)
{
	int retval;
...
EXPORT_SYMBOL(btn_synaptics_rmi4_prox_hover_finger_en);

static void btn_synaptics_rmi4_prox_attn(struct btn_synaptics_rmi4_data *rmi4_data,
		unsigned char intr_mask)
{
	if (!prox)
		return;
...
static int btn_synaptics_rmi4_prox_init(struct btn_synaptics_rmi4_data *rmi4_data)
{
	int retval;
	...
	prox->prox_dev->id.product = BTN_SYNAPTICS_DRIVER_PRODUCT;
	prox->prox_dev->id.version = BTN_SYNAPTICS_DRIVER_VERSION;
...
static void btn_synaptics_rmi4_prox_remove(struct btn_synaptics_rmi4_data *rmi4_data)
{
	unsigned char attr_count;
...
static void btn_synaptics_rmi4_prox_reset(struct btn_synaptics_rmi4_data *rmi4_data)
{
	if (!prox) {
		btn_synaptics_rmi4_prox_init(rmi4_data);
		return;
	}
...
static void btn_synaptics_rmi4_prox_reinit(struct btn_synaptics_rmi4_data *rmi4_data)
{
	if (!prox)
		return;
...
static void btn_synaptics_rmi4_prox_e_suspend(struct btn_synaptics_rmi4_data *rmi4_data)
{
	if (!prox)
		return;
...
static void btn_synaptics_rmi4_prox_suspend(struct btn_synaptics_rmi4_data *rmi4_data)
{
	if (!prox)
		return;
...
static struct btn_synaptics_rmi4_exp_fn proximity_module = {
	.fn_type = RMI_PROXIMITY,
	.init = btn_synaptics_rmi4_prox_init,
	.remove = btn_synaptics_rmi4_prox_remove,
	.reset = btn_synaptics_rmi4_prox_reset,
	.reinit = btn_synaptics_rmi4_prox_reinit,
	.early_suspend = btn_synaptics_rmi4_prox_e_suspend,
	.suspend = btn_synaptics_rmi4_prox_suspend,
	.resume = NULL,
	.late_resume = NULL,
	.attn = btn_synaptics_rmi4_prox_attn,
};

static int __init rmi4_proximity_module_init(void)
{
	btn_synaptics_rmi4_new_function(&proximity_module, true);
...
{
	btn_synaptics_rmi4_new_function(&proximity_module, false);
...
MODULE_DESCRIPTION("BTN_Synaptics DSX Proximity Module");
