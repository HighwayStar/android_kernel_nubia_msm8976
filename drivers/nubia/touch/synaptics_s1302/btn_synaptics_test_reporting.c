...
struct btn_synaptics_rmi4_f54_handle {
	bool no_auto_cal;
...
struct btn_synaptics_rmi4_f55_handle {
	bool amp_sensor;
...
show_store_prototype(read_capval)
...
static struct btn_synaptics_rmi4_f54_handle *f54;
static struct btn_synaptics_rmi4_f55_handle *f55;

DECLARE_COMPLETION(btn_test_remove_complete);

#define TPLOG_DIR "/sdcard/tp_key_rawdata/"
#define BTN_SYNAPTICS_FORMAT_PATH(path,mdir,name) do{\
                               struct timex txc;\
                               struct rtc_time tm;\
...
static int btn_synaptics_write_file(char *file_path, const char *write_buf, int count)
{
	struct file *file_p;
	mm_segment_t old_fs;
	int vfs_retval = -EINVAL;

	pr_debug("write infomation : size =%d\n", count);
	if (NULL == file_path) {
		pr_err("file_path is NULL\n");
		return -EINVAL;
	}

	file_p = filp_open(file_path, O_CREAT|O_RDWR|O_TRUNC , 0775);
	if (IS_ERR(file_p)) {
...
static void test_set_report_size(void)
{
	...
	struct btn_synaptics_rmi4_data *rmi4_data = f54->rmi4_data;
...
static ssize_t test_sysfs_read_capval_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct btn_synaptics_rmi4_data *rmi4_data = f54->rmi4_data;
	struct btn_synaptics_board_data *board_data = rmi4_data ->hw_if->board_data;
	int retval =0;
	const char cmd[] = {'3', 0};
	int tx_num = f54->tx_assigned;
	int rx_num = f54->rx_assigned;
	...
	for (ii = 0; ii < tx_num; ii++) {
		for (jj = 0; jj < rx_num; jj++) {
			...
			{
				if(board_data->virkey_val[i] != 0xffffffff)
					continue;
				if(ii == board_data->virkey_TX[i] && jj==board_data->virkey_RX[i])
					board_data->virkey_val[i] = *report_data_16;
			}
			report_data_16++;
		}
	}
	...
	for(i = 0; i < board_data->virkey_num;i++)
	{
		...
		board_data->virkey_val[i] = 0xffffffff;
	}
	count += sprintf(buf+count, "\n");
	BTN_SYNAPTICS_FORMAT_PATH(file_save_path,TPLOG_DIR,"key_rawdata");
	btn_synaptics_write_file(file_save_path,buf,count);
	return count;
}


static ssize_t test_sysfs_read_capval_store(struct device *dev,
		struct device_attribute *attr, const char *buf,size_t count)
{
	return count;
}
...
//capacitance should < 2.3 pF
	...
	//tx_assigned = 18, but only 17 is useful
	//rx_assigned = 33, but only 30 is useful
	for (ii = 0; ii < tx_num -1; ii++) {
		for (jj = 0; jj < (rx_num - 1); jj++) {
			//skip final 2 value of rx, it is icon value
			if (jj >= (rx_num - 3)) {
				report_data_16++;
				continue;
			}
...
static void test_f54_set_regs(struct btn_synaptics_rmi4_data *rmi4_data,
...
static void test_f55_init(struct btn_synaptics_rmi4_data *rmi4_data)
{
	int retval;
	unsigned char ii;
	unsigned char rx_electrodes = f54->query.num_of_rx_electrodes;
...
static void test_f55_set_regs(struct btn_synaptics_rmi4_data *rmi4_data,
		struct btn_synaptics_rmi4_fn_desc *fd,
		unsigned char page)
{
...
static int test_scan_pdt(void)
{
	...
	for (page = 0; page < PAGES_TO_SERVICE; page++) {
		for (addr = PDT_START; addr > PDT_END; addr -= PDT_ENTRY_SIZE) {
			...
			case BTN_SYNAPTICS_RMI4_F54:
				test_f54_set_regs(rmi4_data,
						&rmi_fd, intr_count, page);
				f54found = true;
				break;
			case BTN_SYNAPTICS_RMI4_F55:
...
static void btn_synaptics_rmi4_test_attn(struct btn_synaptics_rmi4_data *rmi4_data,
		unsigned char intr_mask)
{
	if (!f54)
		return;
...
static int btn_synaptics_rmi4_test_init(struct btn_synaptics_rmi4_data *rmi4_data)
{
	int retval;
#ifdef NUBIA_TP_NODE_IN_KERNEL
...
static void btn_synaptics_rmi4_test_remove(struct btn_synaptics_rmi4_data *rmi4_data)
{
	if (!f54)
		goto exit;
...
exit:
	complete(&btn_test_remove_complete);

	return;
}

static void btn_synaptics_rmi4_test_reset(struct btn_synaptics_rmi4_data *rmi4_data)
{
	int retval;

	if (!f54) {
		btn_synaptics_rmi4_test_init(rmi4_data);
		return;
	}
...
static struct btn_synaptics_rmi4_exp_fn test_module = {
	.fn_type = RMI_TEST_REPORTING,
	.init = btn_synaptics_rmi4_test_init,
	.remove = btn_synaptics_rmi4_test_remove,
	.reset = btn_synaptics_rmi4_test_reset,
	.reinit = NULL,
	.early_suspend = NULL,
	.suspend = NULL,
	.resume = NULL,
	.late_resume = NULL,
	.attn = btn_synaptics_rmi4_test_attn,
};

static int __init rmi4_test_module_init(void)
{
	btn_synaptics_rmi4_new_function(&test_module, true);
...
{
	btn_synaptics_rmi4_new_function(&test_module, false);

	wait_for_completion(&btn_test_remove_complete);
...
MODULE_DESCRIPTION("BTN_Synaptics DSX Test Reporting Module");
