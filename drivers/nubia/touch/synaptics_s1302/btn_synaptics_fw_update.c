...
#define FW_IMAGE_NAME "nubia_tp_btn_jdi_fw.img"
#define FW_IMAGE_NAME_W "nubia_tp_jdi_fw.img"
...
static ssize_t fwu_sysfs_start_update_show(struct device *dev,
		struct device_attribute *attr, char *buf);
...
static ssize_t nubia_ic_ver_show(struct kobject *kobj,
		struct kobj_attribute *attr, char *buf);

static ssize_t nubia_start_update_store(struct kobject *kobj,
...
struct btn_synaptics_rmi4_fwu_handle {
	...
	struct btn_synaptics_rmi4_fn_desc f34_fd;
	struct btn_synaptics_rmi4_fn_desc f35_fd;
...
static struct device_attribute attrs[] = {
	...
	__ATTR(ic_ver, S_IRUGO,
			...
			btn_synaptics_rmi4_store_error),
	__ATTR(start_update, (S_IRUGO | S_IWUSR),
			fwu_sysfs_start_update_show,
...
static struct kobj_attribute nubia_tp_attrs[] = {
	__ATTR(ic_ver, S_IRUGO,
			nubia_ic_ver_show,
			NULL),
	__ATTR(start_update, S_IWUSR,
			NULL,
			nubia_start_update_store),
};
#endif

static struct btn_synaptics_rmi4_fwu_handle *fwu;
...
{
	struct btn_synaptics_rmi4_data *rmi4_data = fwu->rmi4_data;
...
static int fwu_start_reflash(void)
{
	...
	pr_err("%s:product id = %s\n", __func__, rmi->product_id_string);

	if (fwu->image == NULL) {
		if(!strncmp(rmi->product_id_string, "NUBIA-JDI", 9)) {
			pr_err("%s:synaptics tp is jdi !\n", __func__);
			retval = secure_memcpy(fwu->image_name, MAX_IMAGE_NAME_LEN,
			FW_IMAGE_NAME_W, sizeof(FW_IMAGE_NAME_W),
			sizeof(FW_IMAGE_NAME_W));
		}
		else {
			pr_err("%s:synaptics tp is boe !\n", __func__);
			retval = secure_memcpy(fwu->image_name, MAX_IMAGE_NAME_LEN,
...
EXPORT_SYMBOL(btn_synaptics_fw_updater);
		...
		struct device_attribute *attr, char *buf)
{
	...
	if (!do_once)
		return snprintf(buf, PAGE_SIZE, "only need do once !\n");
	do_once = 0;

	btn_synaptics_fw_updater(NULL);
	return snprintf(buf, PAGE_SIZE, "synaptics firmware update finish !\n");
}
...
{
	int retval;
	pr_err("[TP]%s: synaptics firmware update start ! !\n", __func__);
	retval = btn_synaptics_fw_updater(NULL);
	return count;
}
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	retval = btn_synaptics_fw_updater(fwu->image);
	if (retval < 0) {
		dev_err(rmi4_data->pdev->dev.parent,
				"%s: Failed to do reflash\n",
				__func__);
...
static void btn_synaptics_rmi4_fwu_attn(struct btn_synaptics_rmi4_data *rmi4_data,
		unsigned char intr_mask)
{
	if (!fwu)
		return;
...
static int btn_synaptics_rmi4_fwu_init(struct btn_synaptics_rmi4_data *rmi4_data)
{
	int retval;
...
static void btn_synaptics_rmi4_fwu_remove(struct btn_synaptics_rmi4_data *rmi4_data)
{
	unsigned char attr_count;
...
static void btn_synaptics_rmi4_fwu_reset(struct btn_synaptics_rmi4_data *rmi4_data)
{
	int retval;

	if (!fwu) {
		btn_synaptics_rmi4_fwu_init(rmi4_data);
		return;
	}
...
static struct btn_synaptics_rmi4_exp_fn fwu_module = {
	.fn_type = RMI_FW_UPDATER,
	.init = btn_synaptics_rmi4_fwu_init,
	.remove = btn_synaptics_rmi4_fwu_remove,
	.reset = btn_synaptics_rmi4_fwu_reset,
	.reinit = NULL,
	.early_suspend = NULL,
	.suspend = NULL,
	.resume = NULL,
	.late_resume = NULL,
	.attn = btn_synaptics_rmi4_fwu_attn,
};

static int __init rmi4_fw_update_module_init(void)
{
	btn_synaptics_rmi4_new_function(&fwu_module, true);
...
{
	btn_synaptics_rmi4_new_function(&fwu_module, false);
...
MODULE_DESCRIPTION("BTN_Synaptics DSX FW Update Module");
