...
static int btn_synaptics_rmi4_spi_set_page(struct btn_synaptics_rmi4_data *rmi4_data,
		unsigned short addr)
{
	int retval;
	unsigned int index;
...
static int btn_synaptics_rmi4_spi_read(struct btn_synaptics_rmi4_data *rmi4_data,
		unsigned short addr, unsigned char *data, unsigned short length)
{
	...
	retval = btn_synaptics_rmi4_spi_set_page(rmi4_data, addr);
...
static int btn_synaptics_rmi4_spi_write(struct btn_synaptics_rmi4_data *rmi4_data,
		unsigned short addr, unsigned char *data, unsigned short length)
{
	int retval;
	unsigned int index;
...
static struct btn_synaptics_bus_access bus_access = {
	.type = BUS_SPI,
	.read = btn_synaptics_rmi4_spi_read,
	.write = btn_synaptics_rmi4_spi_write,
};

static struct btn_synaptics_hw_interface hw_if;

static struct platform_device *btn_synaptics_spi_device;

static void btn_synaptics_rmi4_spi_dev_release(struct device *dev)
{
	kfree(btn_synaptics_spi_device);

	return;
}

static int btn_synaptics_rmi4_spi_probe(struct spi_device *spi)
{
	int retval;
	...
	btn_synaptics_spi_device = kzalloc(
			sizeof(struct platform_device),
			GFP_KERNEL);
	if (!btn_synaptics_spi_device) {
		dev_err(&spi->dev,
				"%s: Failed to allocate memory for btn_synaptics_spi_device\n",
				__func__);
		return -ENOMEM;
	}
	...
	btn_synaptics_spi_device->name = PLATFORM_DRIVER_NAME;
	btn_synaptics_spi_device->id = 0;
	btn_synaptics_spi_device->num_resources = 0;
	btn_synaptics_spi_device->dev.parent = &spi->dev;
	btn_synaptics_spi_device->dev.platform_data = &hw_if;
	btn_synaptics_spi_device->dev.release = btn_synaptics_rmi4_spi_dev_release;

	retval = platform_device_register(btn_synaptics_spi_device);
	if (retval) {
		dev_err(&spi->dev,
...
static int btn_synaptics_rmi4_spi_remove(struct spi_device *spi)
{
	platform_device_unregister(btn_synaptics_spi_device);
...
static struct spi_driver btn_synaptics_rmi4_spi_driver = {
	.driver = {
		.name = SPI_DRIVER_NAME,
		.owner = THIS_MODULE,
		.of_match_table = btn_synaptics_rmi4_of_match_table,
	},
	.probe = btn_synaptics_rmi4_spi_probe,
	.remove = btn_synaptics_rmi4_spi_remove,
};


int btn_synaptics_rmi4_bus_init(void)
{
	return spi_register_driver(&btn_synaptics_rmi4_spi_driver);
}
EXPORT_SYMBOL(btn_synaptics_rmi4_bus_init);

void btn_synaptics_rmi4_bus_exit(void)
{
	spi_unregister_driver(&btn_synaptics_rmi4_spi_driver);
...
MODULE_DESCRIPTION("BTN_Synaptics DSX SPI Bus Support Module");
