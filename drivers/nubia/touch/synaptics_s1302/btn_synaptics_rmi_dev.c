...
static int rmidev_sysfs_irq_enable(struct btn_synaptics_rmi4_data *rmi4_data,
		bool enable)
{
	int retval = 0;
		...
		char *buf, loff_t pos, size_t count)
{
	...
	struct btn_synaptics_rmi4_data *rmi4_data = rmidev->rmi4_data;
		...
		size_t count, loff_t *f_pos)
{
	...
	retval = btn_synaptics_rmi4_reg_read(rmidev->rmi4_data,
			*f_pos,
			rmidev->tmpbuf,
			count);
	if (retval < 0)
		...
		size_t count, loff_t *f_pos)
{
	...
	retval = btn_synaptics_rmi4_reg_write(rmidev->rmi4_data,
			*f_pos,
			rmidev->tmpbuf,
			count);
	if (retval >= 0)
...
static void rmidev_attn(struct btn_synaptics_rmi4_data *rmi4_data,
		unsigned char intr_mask)
{
	if (!rmidev)
		return;
...
static int rmidev_init_device(struct btn_synaptics_rmi4_data *rmi4_data)
{
	int retval;
	dev_t dev_no;
...
static void rmidev_remove_device(struct btn_synaptics_rmi4_data *rmi4_data)
{
	unsigned char attr_count;
	struct rmidev_data *dev_data;
	const struct btn_synaptics_board_data *bdata =
...
static struct btn_synaptics_rmi4_exp_fn rmidev_module = {
	.fn_type = RMI_DEV,
...
{
	btn_synaptics_rmi4_new_function(&rmidev_module, true);
...
{
	btn_synaptics_rmi4_new_function(&rmidev_module, false);
...
MODULE_DESCRIPTION("BTN_Synaptics DSX RMI Dev Module");
