...
#define FW_IMAGE_NAME_J "nubia_tp_jdi_fw.img"
#define FW_IMAGE_NAME_O "nubia_tp_ofilm_fw.img"
...
static int fwu_start_reflash(void)
{
	...
	if (rmi->product_id_string == NULL)
		retval = -EPERM;

	if (fwu->image == NULL) {
		if ((!strncmp(rmi->product_id_string, "NX589-JDI", 9)) ||
				(!strncmp(rmi->product_id_string, "NX569-JDI", 9)) ||
				(!strncmp(rmi->product_id_string, "s3320", 5))) {
			pr_err("%s:synaptics tp is jdi !\n", __func__);
			retval = secure_memcpy(fwu->image_name, MAX_IMAGE_NAME_LEN,
			FW_IMAGE_NAME_J, sizeof(FW_IMAGE_NAME_J),
			sizeof(FW_IMAGE_NAME_J));
		} else if ((!strncmp(rmi->product_id_string, "NUBIA-CTC", 9)) ||
				(!strncmp(rmi->product_id_string, "NX589-CTC", 9))) {
			pr_err("%s:synaptics tp is ofilm !\n", __func__);
			retval = secure_memcpy(fwu->image_name, MAX_IMAGE_NAME_LEN,
				FW_IMAGE_NAME_O, sizeof(FW_IMAGE_NAME_O),
				sizeof(FW_IMAGE_NAME_O));
		} else {
