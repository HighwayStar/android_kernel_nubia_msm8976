...
show_store_prototype(read_rt2)
show_store_prototype(read_rt3)

show_store_prototype(read_button_capval)
show_store_prototype(home_min_cap)
show_store_prototype(home_max_cap)
show_store_prototype(back_min_cap)
show_store_prototype(back_max_cap)
show_store_prototype(menu_min_cap)
show_store_prototype(menu_max_cap)
show_store_prototype(base_range_cap)
show_store_prototype(keylooptime)

#ifdef NUBIA_TP_NODE_IN_KERNEL
static ssize_t nubia_ic_detect_show(struct kobject *kobj,
...
static struct attribute *attrs[] = {
	...
	attrify(read_rt3),
	attrify(ic_detect),
	attrify(read_button_capval),
	attrify(home_min_cap),
	attrify(home_max_cap),
	attrify(back_min_cap),
	attrify(back_max_cap),
	attrify(menu_min_cap),
	attrify(menu_max_cap),
	attrify(base_range_cap),
	attrify(keylooptime),
	NULL,
};
...
static struct kobj_attribute nubia_tp_attrs[] = {
	__ATTR(ic_detect, S_IRUGO,
			nubia_ic_detect_show,
			NULL),
};
#endif
...
ssize_t nubia_force_cal(void)
{
	...
	retval = test_do_command(COMMAND_FORCE_CAL);
	if (retval < 0) {
		pr_err("%s: Failed to do force cal\n",
				__func__);
		goto exit;
	}
...
static ssize_t test_sysfs_read_button_capval_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct synaptics_rmi4_data *rmi4_data = f54->rmi4_data;
	struct synaptics_dsx_board_data *board_data = rmi4_data ->hw_if->board_data;
	...
	board_data->virkey_val[0]  = report_data_16[ ( tx_num -1 ) *  rx_num + rx_num - 2 ];
	board_data->virkey_val[1]  = report_data_16[ ( tx_num -1 ) *  rx_num + rx_num - 3 ];
	board_data->virkey_val[2]  = report_data_16[ ( tx_num -1 ) *  rx_num + rx_num - 1 ];
	count = sprintf(buf+count, "%d:",board_data->virkey_num);

	for(i = 0; i < board_data->virkey_num;i++)
	{
		virkey_testresult = board_data->virkey_val[i] >  board_data->virkey_minval[i] && board_data->virkey_val[i] < board_data->virkey_maxval[i];
		if(i == board_data->virkey_num - 1)
			count += sprintf(buf+count, "%s %d %d %d %d",board_data->virkey_name[i],board_data->virkey_maxval[i]
					,board_data->virkey_minval[i],board_data->virkey_val[i],virkey_testresult);
		else
			count += sprintf(buf+count, "%s %d %d %d %d, ",board_data->virkey_name[i],board_data->virkey_maxval[i]
					,board_data->virkey_minval[i],board_data->virkey_val[i],virkey_testresult);
	}
...
static ssize_t test_sysfs_read_button_capval_store(struct device *dev,
		struct device_attribute *attr, const char *buf,size_t count)
{
	return count;
}

static ssize_t test_sysfs_base_range_cap_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct synaptics_rmi4_data *rmi4_data = f54->rmi4_data;
	struct synaptics_dsx_board_data *board_data = rmi4_data ->hw_if->board_data;
	int count = 0;
	if(board_data->virkeyrange > 0)
		count += sprintf(buf+count, "%d\n",board_data->virkeyrange);
	return count;
}

static ssize_t test_sysfs_base_range_cap_store(struct device *dev,
		struct device_attribute *attr, const char *buf,size_t count)
{
	return count;
}

static ssize_t test_sysfs_keylooptime_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct synaptics_rmi4_data *rmi4_data = f54->rmi4_data;
	struct synaptics_dsx_board_data *board_data = rmi4_data ->hw_if->board_data;
	int count = 0;
	if(board_data->keylooptime > 0)
		count += sprintf(buf+count, "%d\n",board_data->keylooptime);
	return count;
}

static ssize_t test_sysfs_keylooptime_store(struct device *dev,
		struct device_attribute *attr, const char *buf,size_t count)
{
	return count;
}

static ssize_t test_sysfs_home_min_cap_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct synaptics_rmi4_data *rmi4_data = f54->rmi4_data;
	struct synaptics_dsx_board_data *board_data = rmi4_data ->hw_if->board_data;
	int count = 0;
	if(board_data->virkey_mincap[0]> 0)
		count += sprintf(buf+count, "%d\n",board_data->virkey_mincap[0]);
	return count;
}

static ssize_t test_sysfs_home_min_cap_store(struct device *dev,
		struct device_attribute *attr, const char *buf,size_t count)
{
	return count;
}

static ssize_t test_sysfs_home_max_cap_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct synaptics_rmi4_data *rmi4_data = f54->rmi4_data;
	struct synaptics_dsx_board_data *board_data = rmi4_data ->hw_if->board_data;
	int count = 0;
	if(board_data->virkey_maxcap[0]> 0)
		count += sprintf(buf+count, "%d\n",board_data->virkey_maxcap[0]);
	return count;
}

static ssize_t test_sysfs_home_max_cap_store(struct device *dev,
		struct device_attribute *attr, const char *buf,size_t count)
{
	return count;
}

static ssize_t test_sysfs_menu_max_cap_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct synaptics_rmi4_data *rmi4_data = f54->rmi4_data;
	struct synaptics_dsx_board_data *board_data = rmi4_data ->hw_if->board_data;
	int count = 0;
	if(board_data->virkey_maxcap[1]> 0)
		count += sprintf(buf+count, "%d\n",board_data->virkey_maxcap[1]);
	return count;
}

static ssize_t test_sysfs_menu_max_cap_store(struct device *dev,
		struct device_attribute *attr, const char *buf,size_t count)
{
	return count;
}

static ssize_t test_sysfs_menu_min_cap_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct synaptics_rmi4_data *rmi4_data = f54->rmi4_data;
	struct synaptics_dsx_board_data *board_data = rmi4_data ->hw_if->board_data;
	int count = 0;
	if(board_data->virkey_mincap[1]> 0)
		count += sprintf(buf+count, "%d\n",board_data->virkey_mincap[1]);
	return count;
}

static ssize_t test_sysfs_menu_min_cap_store(struct device *dev,
		struct device_attribute *attr, const char *buf,size_t count)
{
	return count;
}

static ssize_t test_sysfs_back_max_cap_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct synaptics_rmi4_data *rmi4_data = f54->rmi4_data;
	struct synaptics_dsx_board_data *board_data = rmi4_data ->hw_if->board_data;
	int count = 0;
	if(board_data->virkey_maxcap[2]> 0)
		count += sprintf(buf+count, "%d\n",board_data->virkey_maxcap[2]);
	return count;
}

static ssize_t test_sysfs_back_max_cap_store(struct device *dev,
		struct device_attribute *attr, const char *buf,size_t count)
{
	return count;
}

static ssize_t test_sysfs_back_min_cap_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct synaptics_rmi4_data *rmi4_data = f54->rmi4_data;
	struct synaptics_dsx_board_data *board_data = rmi4_data ->hw_if->board_data;
	int count = 0;
	if(board_data->virkey_mincap[2]> 0)
		count += sprintf(buf+count, "%d\n",board_data->virkey_mincap[2]);
	return count;
}

static ssize_t test_sysfs_back_min_cap_store(struct device *dev,
		struct device_attribute *attr, const char *buf,size_t count)
{
	return count;
}

static ssize_t test_sysfs_read_rt2_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	unsigned int ii;
	unsigned int jj;
...
static ssize_t test_sysfs_read_rt2_store(struct device *dev,
		struct device_attribute *attr, const char *buf,size_t count)
{
	return count;
}

static ssize_t test_sysfs_read_rt3_store(struct device *dev,
		struct device_attribute *attr, const char *buf,size_t count)
{
	return count;
}
...
static ssize_t test_sysfs_read_rt3_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	unsigned int ii;
	unsigned int jj;
...
static int read_report_select_type(unsigned long type)
{
	int retval;
	struct synaptics_rmi4_data *rmi4_data = f54->rmi4_data;
	unsigned char data;
	unsigned long setting;
	unsigned char timeout = GET_REPORT_TIMEOUT_S * 50;
	unsigned char timeout_count;
	unsigned char command;
	unsigned char device_ctrl;

//step1-5 is like function test_sysfs_read_report_store

//step1
//test_sysfs_report_type_store
	setting = type;
	mutex_lock(&f54->status_mutex);
	...
	if (retval < 0){
		dev_err(rmi4_data->pdev->dev.parent,
				"%s: test_check_for_idle_status failed\n",
				__func__);
		mutex_unlock(&f54->status_mutex);
		goto error;
	}
...
//step2
//test_sysfs_do_preparation_store
	retval = test_check_for_idle_status();
	if (retval < 0){
		mutex_unlock(&f54->status_mutex);
		goto error;
...
static ssize_t test_sysfs_ic_detect_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	int retval;
	int ii;
	...
	int rx_vk = 3;
	int tx_vk = 1;
//nubia jdi incell tp no need 7 & 17 test
#ifdef IC_DETECT_NO_NEED_7_AND_17
	short low_rx_rx_diagonal = 921;
	short up_rx_rx_diagonal = 1127;
	short low_rx_rx = -102;
	short up_rx_rx = 102;
#endif
...
#define PBUF_LEN 1024
        ...
        if (retval < 0) {
	        dev_err(rmi4_data->pdev->dev.parent, "%s: Failed to select report %d\n", __func__, F54_FULL_RAW_CAP_NO_RX_COUPLING);
	        goto detect_done;
        }

        report_data_16 = (short *)f54->report_data;
        for (ii = 0; ii < tx_num - tx_vk; ii++) {
	        for (jj = 0; jj < rx_num - rx_vk; jj++) {
			if(report_data_16[ ii * rx_num + jj ]  > max_cap)
			{
				max_cap = report_data_16[ ii * rx_num + jj ] ;
			}
			if(report_data_16[ ii * rx_num + jj ]  < min_cap)
			{
...
step1_done:
       if ( retval >= 0 && test_key == 1 )
       {
                //asume  tx be 1 line; rx = 3 line
                if (  \
                    (report_data_16[ ( tx_num -1 ) *  rx_num + rx_num - 3 ] < key_1[0] || report_data_16[ ( tx_num -1 ) *  rx_num + rx_num - 3 ] > key_1[1] )  || \
                    (report_data_16[ ( tx_num -1 ) *  rx_num + rx_num - 2 ] < key_2[0] || report_data_16[ ( tx_num -1 ) *  rx_num + rx_num - 2 ] > key_2[1] )  || \
                    (report_data_16[ ( tx_num -1 ) *  rx_num + rx_num - 1 ] < key_3[0] || report_data_16[ ( tx_num -1 ) *  rx_num + rx_num - 1 ] > key_3[1] )   \
                    )
		{
			dev_err(rmi4_data->pdev->dev.parent, "%s: test fail\n", __func__);
			retval = -EINVAL;
		}
        }

        if ( retval < 0 || always_log )
        {
	        dev_err(rmi4_data->pdev->dev.parent, "%s: test , key1=%d, key2=%d, key3=%d,cap_range[%d , %d]\n", \
	                            __func__, \
	                            report_data_16[ ( tx_num -1 ) *  rx_num + rx_num - 3 ], \
		...
		for (ii = 0; ii < tx_num; ii++) {
			...
			for (jj = 0; jj < rx_num; jj++) {
				len = snprintf(pb, 10, "%d  ", report_data_16[ii * rx_num + jj]);
				if (len < 0 )
				{
					dev_err(rmi4_data->pdev->dev.parent, "%s: print error", __func__);
					goto detect_done;
				}
				tlen += len;
                    if( tlen >=  PBUF_LEN -  11 )
				{
					dev_err(rmi4_data->pdev->dev.parent, "%s: print buffer lenght error", __func__);
					goto detect_done;
				}
				pb += len;
			}
			dev_info(rmi4_data->pdev->dev.parent, "%s %s", __func__, print_buf);
		}
        }
        ...
        //step 2
	retval = read_report_select_type(F54_TX_TO_TX_SHORTS);
	if (retval < 0) {
		dev_err(rmi4_data->pdev->dev.parent, "%s: Failed to select report %d\n", __func__, F54_TX_TO_TX_SHORTS);
		goto detect_done;
	}
	...
	//step2_done:
        if ( retval < 0 || always_log )
        {
	        pr_err("%s:step 2 details\n", __func__);
		for (ii= 0; ii < f54->report_size; ii++) {
		    dev_err(rmi4_data->pdev->dev.parent,"%s %d\n", __func__, f54->report_data[ii] != 0);
		}
        }
#ifdef IC_DETECT_NO_NEED_7_AND_17
	//RX_TO_RX test need combine 7 and 17
	//step 3: type 7, echo 7
	retval = read_report_select_type(F54_RX_TO_RX_SHORTS_1);
	if (retval < 0){
		pr_err("%s:read report select type 7 error !\n", __func__);
		goto exit;
	}

	//step 3: type 7, cat 7
	//a. should between 1024 * 1.1 and 1024 * 0.9  --at diagonal
	//b. should between 1024 * 0.1 and 1024 * (-0.1) --other position
	report_data_16 = (short *)f54->report_data;
	for (ii = 0; ii < tx_num; ii++) {
		for (jj = 0; jj < (rx_num - 1); jj++) {
			if(!(( (*report_data_16) < up_rx_rx_diagonal && (*report_data_16) > low_rx_rx_diagonal)
				|| ((*report_data_16) < up_rx_rx && (*report_data_16) > low_rx_rx))){
				pr_err("%s:exist wrong report 7 data, test exit !\n", __func__);
				goto exit;
			}
			report_data_16++;
		}
	...
	//step 3: type 17, echo 17
	retval = read_report_select_type(F54_RX_TO_RX_SHORTS_2);
	if (retval < 0){
		pr_err("%s:read report select type 17 error !\n", __func__);
		goto exit;
	}

	//step 3: type 17, cat 17
	report_data_16 = (short *)f54->report_data;
	for (ii = 0; ii < tx_num; ii++) {
		for (jj = 0; jj < (rx_num - 1); jj++) {
			if(!(( (*report_data_16) < up_rx_rx_diagonal && (*report_data_16) > low_rx_rx_diagonal)
				|| ((*report_data_16) < up_rx_rx && (*report_data_16) > low_rx_rx))){
				pr_err("%s:exist wrong report 17 data, test exit !\n", __func__);
				goto exit;
			}
			report_data_16++;
		}
		...
		struct kobj_attribute *attr, char *buf)
{
	...
	int rx_num = f54->rx_assigned;
	short *report_data_16;
	short low_cap = 1000; //min value
	short up_cap = 3000; //max value
//nubia jdi incell tp no need 7 & 17 test
#ifdef IC_DETECT_NO_NEED_7_AND_17
	short low_rx_rx_diagonal = 921;
	short up_rx_rx_diagonal = 1127;
...
//step 1: type 3, echo 3
	retval = read_report_select_type(F54_RAW_16BIT_IMAGE);
	if (retval < 0){
		pr_err("%s:read report select type 3 error !\n", __func__);
		goto exit;
	}

//step 1: type 3, cat 3
//capacitance should < 2.3 pF
	report_data_16 = (short *)f54->report_data;

	for (ii = 0; ii < tx_num; ii++) {
		for (jj = 0; jj < (rx_num - 1); jj++) {
			if( (*report_data_16) < low_cap || (*report_data_16)  > up_cap){
				pr_err("%s:exist wrong report 3 data, test exit !\n", __func__);
				goto exit;
			}
			report_data_16++;
		}
		//skip final value of rx
		report_data_16++;
	}

//step 2: type 5, echo 5
	retval = read_report_select_type(F54_TX_TO_TX_SHORTS);
	if (retval < 0){
		pr_err("%s:read report select type 5 error !\n", __func__);
		goto exit;
	}

//step 2: type 5, cat 5
//ensure report date is 0
	for (ii= 0; ii < f54->report_size; ii++) {
		if(f54->report_data[ii] != 0){
			pr_err("%s:report type 5, data should be 0, test exit !\n", __func__);
			goto exit;
		}
	}
