...
#ifdef CONFIG_TOUCHSCREEN_NUBIA_USER_TP_UPDATE
#include <linux/kthread.h>
#include <linux/security.h>
#include <linux/vmalloc.h>
#endif
#define FW_IMAGE_NAME "nubia_tp_boe_fw.img"
#define FW_IMAGE_NAME_W "nubia_tp_jdi_fw.img"
#define FW_IMAGE_NAME_OCTCR "nubia_tp_ofilm_fw_ctcr.img"
#define FW_IMAGE_NAME_OCTCT "nubia_tp_ofilm_fw_ctct.img"
...
#ifdef CONFIG_TOUCHSCREEN_NUBIA_USER_TP_UPDATE
static ssize_t nubia_user_start_update_show(struct device *dev,
		struct device_attribute *attr, char *buf);
#endif
...
#ifdef CONFIG_TOUCHSCREEN_NUBIA_USER_TP_UPDATE
	__ATTR(start_user_update, S_IRUGO,
			nubia_user_start_update_show,
			synaptics_rmi4_store_error),
#endif
};
...
#ifdef CONFIG_TOUCHSCREEN_NUBIA_FWID_FORCE_UPDATE
	pr_err("%s: nubia touch fw update no check id\n", __func__);
...
#ifdef CONFIG_TOUCHSCREEN_NUBIA_USER_TP_UPDATE
static int nubia_user_fwu_start_reflash(void)
{
	char *img_data;
	...
	if (fwu->image == NULL) {
		...
		snprintf(img_path, 100, "%s/%s","/mnt/sdcard", fwu->image_name);
		//pr_info("[TP]path=%s\n",img_path);
...
#ifdef CONFIG_TOUCHSCREEN_NUBIA_USER_TP_UPDATE
int nubia_user_synaptics_fw_updater(const unsigned char *fw_data)
{
	int retval;
	...
	retval = nubia_user_fwu_start_reflash();
		...
		struct device_attribute *attr, char *buf)
{
	...
	int retval;
	pr_err("[TP]%s: nubia user synaptics firmware update start ! !\n", __func__);
	retval = nubia_user_synaptics_fw_updater(NULL);
	if(!retval)
	{
		pr_err("[TP]:nubia user update success\n");
		return snprintf(buf, PAGE_SIZE, "[TP]:nubia user update success\n");
	}
	else
	{
		pr_err("[TP]:nubia user update fail\n");
		return snprintf(buf, PAGE_SIZE, "[TP]:nubia user update fail\n");
	}
