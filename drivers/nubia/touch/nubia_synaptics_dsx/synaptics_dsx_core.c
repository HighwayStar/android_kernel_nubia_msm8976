...
#define SYNAPTICS_VIO_MIN_UV	1800000
#define SYNAPTICS_VIO_MAX_UV	1800000
...
static ssize_t synaptics_rmi4_back_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count);
static ssize_t synaptics_rmi4_key_isfactory_show(struct device *dev,
		struct device_attribute *attr, char *buf);

static ssize_t synaptics_rmi4_key_isfactory_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count);
...
static struct device_attribute attrs[] = {
	...
	__ATTR(key_isfactory, (S_IRUGO | S_IWUSR),
			synaptics_rmi4_key_isfactory_show,
			synaptics_rmi4_key_isfactory_store),
...
static ssize_t synaptics_rmi4_key_isfactory_store(struct device *dev,
	...
	if(input == 1)
		rmi4_data->key_isfactory = true;
	else
		rmi4_data->key_isfactory = false;
	pr_err("key_isfactory is %d\n", rmi4_data->key_isfactory);
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (rmi4_data->f11_wakeup_gesture || rmi4_data->f12_wakeup_gesture) {
		pr_err("%s:%d!current process=%s pid=%d known pid=%d, rmi4_data->enable_wakeup_gesture = %d\n",
				__func__, __LINE__, task->comm, current->tgid, pid, input);
...
#else
		touch_count++;
		if(rmi4_data ->hw_if->board_data->key_isfactory && rmi4_data->key_isfactory)
		{
			if(button == 0){
				pr_err("report key KEY_F5 %s\n",status==0?"UP":"DOWN");
				input_report_key(rmi4_data->input_dev,
						KEY_F5,
						status);
			}
			if(button == 1){
				pr_err("report key KEY_F4 %s\n",status==0?"UP":"DOWN");
				input_report_key(rmi4_data->input_dev,
						KEY_F4,
						status);
			}
			if(button == 2){
				pr_err("report key KEY_F6 %s\n",status==0?"UP":"DOWN");
				input_report_key(rmi4_data->input_dev,
						KEY_F6,
						status);
			}
...
flash_prog_mode:
	...
	pr_err("%s:%d!current process=%s pid=%d known pid=%d,rmi4_data->enable_wakeup_gesture = %d\n",
			__func__, __LINE__, task->comm, current->tgid, pid,rmi4_data->enable_wakeup_gesture);
...
//nubia for palm sleep
	...
	if (f1a) {
		...
		input_set_capability(rmi4_data->input_dev, EV_KEY, KEY_F4);
		set_bit(KEY_F5, rmi4_data->input_dev->keybit);
		input_set_capability(rmi4_data->input_dev, EV_KEY, KEY_F5);
		set_bit(KEY_F6, rmi4_data->input_dev->keybit);
		input_set_capability(rmi4_data->input_dev, EV_KEY, KEY_F6);
	}
...
static void resume_work_func(struct work_struct *work)
{
	...
	struct synaptics_rmi4_data *rmi4_data =
		container_of(work, struct synaptics_rmi4_data,
			resume_work);
	pr_err("[TP]:resume workqueue start\n");
	...
	pr_err("[TP]:resume workqueue finish\n");
}
#endif
...
exit:
#ifdef NUBIA_SYNAPTICS_RESUME_WORKQUEUE
		queue_work(rmi4_data->resume_wq,
