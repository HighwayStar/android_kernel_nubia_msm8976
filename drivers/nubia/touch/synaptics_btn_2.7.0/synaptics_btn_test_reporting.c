...
struct synaptics_btn_rmi4_f54_handle {
	bool no_auto_cal;
...
struct synaptics_btn_rmi4_f55_handle {
	bool amp_sensor;
	bool extended_amp;
	bool has_force;
...
struct synaptics_btn_rmi4_f21_handle {
	bool has_force;
	unsigned char tx_assigned;
	unsigned char rx_assigned;
...
show_prototype(read_rt2)
show_prototype(read_rt3)
show_prototype(read_btn_delta)
show_prototype(read_btn_raw)
#endif
...
static struct attribute *attrs[] = {
	...
	attrify(read_rt2),
	attrify(read_rt3),
	attrify(read_btn_delta),
	attrify(read_btn_raw),
#endif
	NULL,
};
...
static struct synaptics_btn_rmi4_f54_handle *f54;
static struct synaptics_btn_rmi4_f55_handle *f55;
static struct synaptics_btn_rmi4_f21_handle *f21;
...
static void test_set_report_size(void)
{
	...
	struct synaptics_btn_rmi4_data *rmi4_data = f54->rmi4_data;
		...
		struct device_attribute *attr, char *buf)
{
	...
	short *report_data_16;
	short low_cap = bdata->check_cap_2d_min;
	short up_cap = bdata->check_cap_2d_max;
		...
		struct device_attribute *attr, char *buf)
{
	...
	retval = test_sysfs_read_report_store(dev, attr, type, 1);

	retval = test_sysfs_read_report_show(dev, attr, buf);
...
static ssize_t test_sysfs_read_btn_delta_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	int tx_num = f54->tx_assigned;
	int rx_num = f54->rx_assigned;
	...
	virkey_val[0]  = report_data_16[ ( tx_num -1 ) *  rx_num + rx_num - 1 ];
	virkey_val[1]  = report_data_16[ ( tx_num -1 ) *  rx_num + rx_num - 2 ];
	virkey_val[2]  = report_data_16[ ( tx_num -1 ) *  rx_num + rx_num - 3 ];

	for(i = 0; i < 3; i++) {
		cnt = snprintf(buf, PAGE_SIZE, "btn_delta[%d] = %d\n", i, virkey_val[i]);
		buf += cnt;
		count += cnt;
	}
	return count;
...
static ssize_t test_sysfs_read_btn_raw_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	int tx_num = f54->tx_assigned;
	int rx_num = f54->rx_assigned;
	...
	for(i = 0; i < 3; i++) {
		cnt = snprintf(buf, PAGE_SIZE, "btn_raw[%d] = %d\n", i, virkey_val[i]);
		buf += cnt;
		count += cnt;
	}
	return count;
...
static void test_f54_set_regs(struct synaptics_btn_rmi4_data *rmi4_data,
...
static void test_f55_init(struct synaptics_btn_rmi4_data *rmi4_data)
{
	int retval;
	unsigned char ii;
	unsigned char rx_electrodes;
...
static void test_f55_set_regs(struct synaptics_btn_rmi4_data *rmi4_data,
		struct synaptics_btn_rmi4_fn_desc *fd,
		unsigned char page)
{
...
static void test_f21_init(struct synaptics_btn_rmi4_data *rmi4_data)
{
	int retval;
	unsigned char ii;
...
static void test_f21_set_regs(struct synaptics_btn_rmi4_data *rmi4_data,
		struct synaptics_btn_rmi4_fn_desc *fd,
		unsigned char page)
{
...
static void synaptics_btn_rmi4_test_attn(struct synaptics_btn_rmi4_data *rmi4_data,
		unsigned char intr_mask)
{
	if (!f54)
		return;
...
static int synaptics_btn_rmi4_test_init(struct synaptics_btn_rmi4_data *rmi4_data)
{
	int retval;
...
static void synaptics_btn_rmi4_test_remove(struct synaptics_btn_rmi4_data *rmi4_data)
{
	if (!f54)
		goto exit;
...
static void synaptics_btn_rmi4_test_reset(struct synaptics_btn_rmi4_data *rmi4_data)
{
	int retval;

	if (!f54) {
		synaptics_btn_rmi4_test_init(rmi4_data);
		return;
	}
...
static struct synaptics_btn_rmi4_exp_fn test_module = {
	.fn_type = RMI_TEST_REPORTING,
	.init = synaptics_btn_rmi4_test_init,
	.remove = synaptics_btn_rmi4_test_remove,
	.reset = synaptics_btn_rmi4_test_reset,
	.reinit = NULL,
	.early_suspend = NULL,
	.suspend = NULL,
	.resume = NULL,
	.late_resume = NULL,
	.attn = synaptics_btn_rmi4_test_attn,
};

static int __init rmi4_test_module_init(void)
{
	synaptics_btn_rmi4_new_function(&test_module, true);
...
{
	synaptics_btn_rmi4_new_function(&test_module, false);
...
MODULE_DESCRIPTION("Synaptics Button DSX Test Reporting Module");
