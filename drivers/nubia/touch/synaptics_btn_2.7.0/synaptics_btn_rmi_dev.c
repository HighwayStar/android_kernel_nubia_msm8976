...
#define CHAR_DEVICE_NAME "rmi_btn"
#define DEVICE_CLASS_NAME "rmidev_btn"
#define SYSFS_FOLDER_NAME "rmidev_btn"
#define DEV_NUMBER 1
...
DECLARE_COMPLETION(btn_rmidev_remove_complete);
...
static int rmidev_sysfs_irq_enable(struct synaptics_btn_rmi4_data *rmi4_data,
		bool enable)
{
	int retval = 0;
		...
		char *buf, loff_t pos, size_t count)
{
	...
	struct synaptics_btn_rmi4_data *rmi4_data = rmidev->rmi4_data;
		...
		size_t count, loff_t *f_pos)
{
	...
	retval = synaptics_btn_rmi4_reg_read(rmidev->rmi4_data,
			*f_pos,
			rmidev->tmpbuf,
			count);
	if (retval < 0)
		...
		size_t count, loff_t *f_pos)
{
	...
	retval = synaptics_btn_rmi4_reg_write(rmidev->rmi4_data,
			*f_pos,
			rmidev->tmpbuf,
			count);
	if (retval >= 0)
...
static void rmidev_attn(struct synaptics_btn_rmi4_data *rmi4_data,
		unsigned char intr_mask)
{
	if (!rmidev)
		return;
...
static int rmidev_init_device(struct synaptics_btn_rmi4_data *rmi4_data)
{
	int retval;
	dev_t dev_no;
...
static void rmidev_remove_device(struct synaptics_btn_rmi4_data *rmi4_data)
{
	unsigned char attr_count;
	struct rmidev_data *dev_data;
	const struct synaptics_btn_board_data *bdata =
...
exit:
	complete(&btn_rmidev_remove_complete);

	return;
}

static struct synaptics_btn_rmi4_exp_fn rmidev_module = {
	.fn_type = RMI_DEV,
...
{
	synaptics_btn_rmi4_new_function(&rmidev_module, true);
...
{
	synaptics_btn_rmi4_new_function(&rmidev_module, false);

	wait_for_completion(&btn_rmidev_remove_complete);
...
MODULE_DESCRIPTION("Synaptics Button DSX RMI Dev Module");
