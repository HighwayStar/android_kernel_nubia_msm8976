...
#define APEN_PHYS_NAME "synaptics_btn/active_pen"
...
struct synaptics_btn_rmi4_apen_handle {
	bool apen_present;
...
static struct synaptics_btn_rmi4_apen_handle *apen;
...
static void apen_report(void)
{
	...
	struct synaptics_btn_rmi4_data *rmi4_data = apen->rmi4_data;
...
static int apen_pressure(struct synaptics_btn_rmi4_f12_query_8 *query_8)
{
	int retval;
	unsigned char ii;
...
static void synaptics_btn_rmi4_apen_attn(struct synaptics_btn_rmi4_data *rmi4_data,
		unsigned char intr_mask)
{
	if (!apen)
		return;
...
static int synaptics_btn_rmi4_apen_init(struct synaptics_btn_rmi4_data *rmi4_data)
{
	int retval;
	...
	apen->apen_dev->id.product = SYNAPTICS_BTN_DRIVER_PRODUCT;
	apen->apen_dev->id.version = SYNAPTICS_BTN_DRIVER_VERSION;
...
static void synaptics_btn_rmi4_apen_remove(struct synaptics_btn_rmi4_data *rmi4_data)
{
	if (!apen)
		goto exit;
...
static void synaptics_btn_rmi4_apen_reset(struct synaptics_btn_rmi4_data *rmi4_data)
{
	if (!apen) {
		synaptics_btn_rmi4_apen_init(rmi4_data);
		return;
	}
...
static void synaptics_btn_rmi4_apen_reinit(struct synaptics_btn_rmi4_data *rmi4_data)
{
	if (!apen)
		return;
...
static void synaptics_btn_rmi4_apen_e_suspend(struct synaptics_btn_rmi4_data *rmi4_data)
{
	if (!apen)
		return;
...
static void synaptics_btn_rmi4_apen_suspend(struct synaptics_btn_rmi4_data *rmi4_data)
{
	if (!apen)
		return;
...
static struct synaptics_btn_rmi4_exp_fn active_pen_module = {
	.fn_type = RMI_ACTIVE_PEN,
	.init = synaptics_btn_rmi4_apen_init,
	.remove = synaptics_btn_rmi4_apen_remove,
	.reset = synaptics_btn_rmi4_apen_reset,
	.reinit = synaptics_btn_rmi4_apen_reinit,
	.early_suspend = synaptics_btn_rmi4_apen_e_suspend,
	.suspend = synaptics_btn_rmi4_apen_suspend,
	.resume = NULL,
	.late_resume = NULL,
	.attn = synaptics_btn_rmi4_apen_attn,
};

static int __init rmi4_active_pen_module_init(void)
{
	synaptics_btn_rmi4_new_function(&active_pen_module, true);
...
{
	synaptics_btn_rmi4_new_function(&active_pen_module, false);
...
MODULE_DESCRIPTION("Synaptics Button DSX Active Pen Module");
