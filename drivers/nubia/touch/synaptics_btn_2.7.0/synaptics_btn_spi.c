...
#include <linux/input/synaptics_btn.h>
...
static int synaptics_btn_rmi4_spi_alloc_buf(struct synaptics_btn_rmi4_data *rmi4_data,
...
static int synaptics_btn_rmi4_spi_set_page(struct synaptics_btn_rmi4_data *rmi4_data,
		unsigned short addr)
{
	int retval;
	unsigned int index;
	...
	retval = synaptics_btn_rmi4_spi_alloc_buf(rmi4_data, byte_count,
			byte_count);
	if (retval < 0)
		return retval;
...
static int synaptics_btn_rmi4_spi_read(struct synaptics_btn_rmi4_data *rmi4_data,
		unsigned short addr, unsigned char *data, unsigned int length)
{
	...
	retval = synaptics_btn_rmi4_spi_set_page(rmi4_data, addr);
	...
	if (bdata->byte_delay_us == 0) {
		retval = synaptics_btn_rmi4_spi_alloc_buf(rmi4_data, length,
				2);
	} else {
		retval = synaptics_btn_rmi4_spi_alloc_buf(rmi4_data, length,
				byte_count);
...
static int synaptics_btn_rmi4_spi_write(struct synaptics_btn_rmi4_data *rmi4_data,
		unsigned short addr, unsigned char *data, unsigned int length)
{
	int retval;
	unsigned int index;
...
static struct synaptics_btn_bus_access bus_access = {
	.type = BUS_SPI,
	.read = synaptics_btn_rmi4_spi_read,
	.write = synaptics_btn_rmi4_spi_write,
};

static struct synaptics_btn_hw_interface hw_if;

static struct platform_device *synaptics_btn_spi_device;

static void synaptics_btn_rmi4_spi_dev_release(struct device *dev)
{
	kfree(synaptics_btn_spi_device);

	return;
}

static int synaptics_btn_rmi4_spi_probe(struct spi_device *spi)
{
	int retval;
	...
	synaptics_btn_spi_device = kzalloc(
			sizeof(struct platform_device),
			GFP_KERNEL);
	if (!synaptics_btn_spi_device) {
		dev_err(&spi->dev,
				"%s: Failed to allocate memory for synaptics_btn_spi_device\n",
				__func__);
		return -ENOMEM;
	}
	...
	synaptics_btn_spi_device->name = PLATFORM_DRIVER_NAME;
	synaptics_btn_spi_device->id = 0;
	synaptics_btn_spi_device->num_resources = 0;
	synaptics_btn_spi_device->dev.parent = &spi->dev;
	synaptics_btn_spi_device->dev.platform_data = &hw_if;
	synaptics_btn_spi_device->dev.release = synaptics_btn_rmi4_spi_dev_release;

	retval = platform_device_register(synaptics_btn_spi_device);
	if (retval) {
		dev_err(&spi->dev,
...
static int synaptics_btn_rmi4_spi_remove(struct spi_device *spi)
{
	platform_device_unregister(synaptics_btn_spi_device);

	return 0;
}

static const struct spi_device_id synaptics_btn_rmi4_id_table[] = {
	{SPI_DRIVER_NAME, 0},
	{},
};
MODULE_DEVICE_TABLE(spi, synaptics_btn_rmi4_id_table);
...
static struct spi_driver synaptics_btn_rmi4_spi_driver = {
	.driver = {
		.name = SPI_DRIVER_NAME,
		.owner = THIS_MODULE,
		.of_match_table = synaptics_btn_rmi4_of_match_table,
	},
	.probe = synaptics_btn_rmi4_spi_probe,
	.remove = synaptics_btn_rmi4_spi_remove,
...
{
	return spi_register_driver(&synaptics_btn_rmi4_spi_driver);
}
...
void synaptics_btn_rmi4_bus_exit(void)
{
	...
	spi_unregister_driver(&synaptics_btn_rmi4_spi_driver);
...
MODULE_DESCRIPTION("Synaptics Button DSX SPI Bus Support Module");
