...
#ifdef NUBIA_TOUCH_SYNAPTICS
static ssize_t fwu_sysfs_ic_ver_show(struct device *dev,
		struct device_attribute *attr, char *buf);

static ssize_t fwu_sysfs_start_update_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count);
#endif
...
struct synaptics_btn_rmi4_fwu_handle {
	...
	struct synaptics_btn_rmi4_fn_desc f34_fd;
	struct synaptics_btn_rmi4_fn_desc f35_fd;
...
static struct device_attribute attrs[] = {
	...
	__ATTR(ic_ver, S_IRUGO,
			fwu_sysfs_ic_ver_show,
			synaptics_btn_rmi4_store_error),
	__ATTR(start_update, (S_IWUSR | S_IWGRP),
			synaptics_btn_rmi4_show_error,
			fwu_sysfs_start_update_store),
#endif
};

static struct synaptics_btn_rmi4_fwu_handle *fwu;

DECLARE_COMPLETION(btn_fwu_remove_complete);

DEFINE_MUTEX(btn_fwu_sysfs_mutex);
...
static int fwu_f51_force_data_init(void)
{
	...
	struct synaptics_btn_rmi4_data *rmi4_data = fwu->rmi4_data;
...
static int fwu_start_reflash(void)
{
	...
	struct synaptics_btn_rmi4_device_info *rmi = &(rmi4_data->rmi4_mod_info);
#endif
...
#ifdef NUBIA_TOUCH_SYNAPTICS
	pr_info("%s:synaptics product id = %s\n", __func__, rmi->product_id_string);
#endif


	if (fwu->image == NULL) {
#ifdef NUBIA_TOUCH_SYNAPTICS
		if((!strncmp(rmi->product_id_string, bdata->panel1_product_id1,
				strlen(rmi->product_id_string)) ) ||
				(!strncmp(rmi->product_id_string, bdata->panel1_product_id2,
					strlen(rmi->product_id_string)))) {
			retval = secure_memcpy(fwu->image_name, MAX_IMAGE_NAME_LEN,
				bdata->fw_image_name1, strlen(bdata->fw_image_name1) + 1,
				strlen(bdata->fw_image_name1) + 1);
			if (retval < 0) {
				dev_err(rmi4_data->pdev->dev.parent,
		...
		} else if((!strncmp(rmi->product_id_string, bdata->panel2_product_id1,
				strlen(rmi->product_id_string)) ) ||
				(!strncmp(rmi->product_id_string, bdata->panel2_product_id2,
					strlen(rmi->product_id_string)))) {
			retval = secure_memcpy(fwu->image_name, MAX_IMAGE_NAME_LEN,
				bdata->fw_image_name2, strlen(bdata->fw_image_name2) + 1,
				strlen(bdata->fw_image_name2) + 1);
			if (retval < 0) {
				dev_err(rmi4_data->pdev->dev.parent,
						"%s: Failed to copy image file name\n",
						__func__);
		...
		} else if((!strncmp(rmi->product_id_string, bdata->panel3_product_id1,
				strlen(rmi->product_id_string)) ) ||
				(!strncmp(rmi->product_id_string, bdata->panel3_product_id2,
					strlen(rmi->product_id_string)))) {
			retval = secure_memcpy(fwu->image_name, MAX_IMAGE_NAME_LEN,
				bdata->fw_image_name3, strlen(bdata->fw_image_name3) + 1,
				strlen(bdata->fw_image_name3) + 1);
			if (retval < 0) {
				dev_err(rmi4_data->pdev->dev.parent,
						"%s: Failed to copy image file name\n",
						__func__);
			...
			pr_err("%s:synaptics no available fw image for %s !\n",
					__func__, rmi->product_id_string);
			goto exit;
		}
		if (fwu->image_name != NULL)
			pr_info("%s:synaptics image is %s !\n", __func__, fwu->image_name);
#else
		retval = secure_memcpy(fwu->image_name, MAX_IMAGE_NAME_LEN,
...
int synaptics_btn_fw_updater(const unsigned char *fw_data)
{
	int retval;
...
EXPORT_SYMBOL(synaptics_btn_fw_updater);
		...
		char *buf, loff_t pos, size_t count)
{
	...
	if (!mutex_trylock(&btn_fwu_sysfs_mutex))
		return -EBUSY;
...
exit:
	mutex_unlock(&btn_fwu_sysfs_mutex);
	return retval;
}
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (do_once) {
		pr_debug("%s: synaptics firmware update start ! !\n", __func__);
		retval = synaptics_btn_fw_updater(NULL);
		do_once = false;
	} else {
		pr_err("%s: synaptics firmware update only need do once !\n", __func__);
	}
	return count;
}
#endif
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	retval = synaptics_btn_fw_updater(fwu->image);
	if (retval < 0) {
		dev_err(rmi4_data->pdev->dev.parent,
				"%s: Failed to do reflash\n",
				__func__);
...
{
	unsigned int image_fw_id;
	fwu_get_image_firmware_id(&image_fw_id);
...
static void synaptics_btn_rmi4_fwu_attn(struct synaptics_btn_rmi4_data *rmi4_data,
		unsigned char intr_mask)
{
	if (!fwu)
		return;
...
static int synaptics_btn_rmi4_fwu_init(struct synaptics_btn_rmi4_data *rmi4_data)
{
	int retval;
...
static void synaptics_btn_rmi4_fwu_remove(struct synaptics_btn_rmi4_data *rmi4_data)
{
	unsigned char attr_count;
...
exit:
	complete(&btn_fwu_remove_complete);

	return;
}

static void synaptics_btn_rmi4_fwu_reset(struct synaptics_btn_rmi4_data *rmi4_data)
{
	int retval;

	if (!fwu) {
		synaptics_btn_rmi4_fwu_init(rmi4_data);
		return;
	}
...
static struct synaptics_btn_rmi4_exp_fn fwu_module = {
	.fn_type = RMI_FW_UPDATER,
	.init = synaptics_btn_rmi4_fwu_init,
	.remove = synaptics_btn_rmi4_fwu_remove,
	.reset = synaptics_btn_rmi4_fwu_reset,
	.reinit = NULL,
	.early_suspend = NULL,
	.suspend = NULL,
	.resume = NULL,
	.late_resume = NULL,
	.attn = synaptics_btn_rmi4_fwu_attn,
};

static int __init rmi4_fw_update_module_init(void)
{
	synaptics_btn_rmi4_new_function(&fwu_module, true);
...
{
	synaptics_btn_rmi4_new_function(&fwu_module, false);

	wait_for_completion(&btn_fwu_remove_complete);
...
MODULE_DESCRIPTION("Synaptics Button DSX FW Update Module");
