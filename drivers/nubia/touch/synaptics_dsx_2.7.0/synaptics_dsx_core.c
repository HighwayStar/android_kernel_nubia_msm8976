...
#ifdef NUBIA_TOUCH_SYNAPTICS
bool nubia_enable_wakeup_gesture(void)
{
...
#ifdef NUBIA_TOUCH_SYNAPTICS
static int synaptics_rmi4_enable_reset_gpio(struct synaptics_rmi4_data *rmi4_data,
		bool enable)
{
	int retval = 0;
	const struct synaptics_dsx_board_data *bdata =
	...
	if (enable) {
		if (bdata->reset_gpio >= 0 && !rmi4_data->avdd_always_on
			...
			if (bdata->reset_on_sleep)
				usleep(bdata->reset_on_sleep * 1000);
		}
	} else {
		if (bdata->reset_gpio >= 0 && !rmi4_data->avdd_always_on
				&& !rmi4_data->vdd_always_on) {
			...
			if (bdata->reset_off_sleep)
				usleep(bdata->reset_off_sleep * 1000);
		}
	}
	return retval;
}
static int synaptics_rmi4_enable_reg_gpio(struct synaptics_rmi4_data *rmi4_data,
		bool enable)
{
	int retval = 0;
	const struct synaptics_dsx_board_data *bdata =
	...
	if (enable) {
		if (bdata->power_on_avdd_first) {
			if (bdata->power_gpio_avdd >= 0 && !rmi4_data->avdd_always_on) {
				...
				if (bdata->avdd_seq->post_on_sleep)
			...
			if (bdata->power_gpio_vdd >= 0 && !rmi4_data->vdd_always_on) {
				...
				if (bdata->vdd_seq->post_on_sleep)
		...
		if (bdata->power_off_avdd_first) {
			if (bdata->power_gpio_avdd >= 0 && !rmi4_data->avdd_always_on) {
				...
				if (bdata->avdd_seq->post_off_sleep)
			...
			if (bdata->power_gpio_vdd >= 0 && !rmi4_data->vdd_always_on) {
				...
				if (bdata->vdd_seq->post_off_sleep)
		...
		bool enable)
{
	...
	if (bdata->power_on_gpio_first) {
		retval = synaptics_rmi4_enable_reg_gpio(rmi4_data, true);
		if (retval < 0) {
			dev_err(rmi4_data->pdev->dev.parent,
	...
	if (bdata->power_on_avdd_first) {
		if (rmi4_data->pwr_reg && !rmi4_data->avdd_always_on) {
			...
			if (need_sleep && bdata->avdd_seq->post_on_sleep)
				usleep(bdata->avdd_seq->post_on_sleep * 1000);
		}
		...
		if (rmi4_data->bus_reg && !rmi4_data->vdd_always_on) {
			...
			if (need_sleep && bdata->vdd_seq->post_on_sleep)
				usleep(bdata->vdd_seq->post_on_sleep * 1000);
...
disable_reg:
	if (bdata->power_off_gpio_first) {
		retval = synaptics_rmi4_enable_reg_gpio(rmi4_data, false);
		if (retval < 0) {
			dev_err(rmi4_data->pdev->dev.parent,
...
static void synaptics_rmi4_power_ctrl(struct synaptics_rmi4_data *rmi4_data,
		bool enable)
{
	...
	if (enable) {
		...
		synaptics_rmi4_enable_reset_gpio(rmi4_data, true);
	} else {
		synaptics_rmi4_enable_reset_gpio(rmi4_data, false);
...
static void synaptics_rmi4_reset_work_func(struct work_struct *work)
{
	int ret;
	...
	pr_debug("synaptics reset work queue finished !\n");
...
static int synaptics_rmi4_probe(struct platform_device *pdev)
{
	...
	rmi4_data->reset_workqueue =
			create_singlethread_workqueue("dsx_reset_workqueue");
	INIT_WORK(&rmi4_data->reset_work, synaptics_rmi4_reset_work_func);
#endif
		...
		unsigned long event, void *data)
{
	...
	if (event == FB_EARLY_EVENT_BLANK) {
		...
		if (*transition == FB_BLANK_UNBLANK) {
			synaptics_rmi4_power_ctrl(rmi4_data, true);
			if (bdata->touch_resume_first || rmi4_data->enable_wakeup_gesture) {
		...
		if (*transition == FB_BLANK_POWERDOWN) {
			...
			synaptics_rmi4_power_ctrl(rmi4_data, false);
