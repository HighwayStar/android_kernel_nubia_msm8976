...
#ifdef CONFIG_NUBIA_SENSORS_LOG_ON
...
static int i_three_power_on(struct i_three_chip *chip, bool enable)
{
	...
	if (enable) {
		if(chip->use_gpio) {
			SENSOR_LOG_INFO("Set enable_agpio[%d] to high\n", chip->enable_gpio);
		...
		if (chip->use_gpio) {
			SENSOR_LOG_INFO("Set enable_agpio[%d] to low\n", chip->enable_gpio);
			...
			const struct i2c_device_id *id)
{
	...
	ret = of_get_named_gpio(of_node, "ir_remote,enable-gpio", 0);
	if (ret < 0) {
		chip->use_gpio = false;
		SENSOR_LOG_INFO("Unable to read enable gpio\n");
	} else {
		chip->use_gpio = true;
		chip->enable_gpio = ret;
		ret = gpio_direction_output(chip->enable_gpio, 0);
		if (ret < 0) {
			SENSOR_LOG_ERROR("Set enable gpio[%d] to output failed.\n", chip->enable_gpio);
		}
		ret = gpio_get_value(chip->enable_gpio);
		SENSOR_LOG_INFO("enable gpio[%d] status is %d\n", chip->enable_gpio, ret);
	}
	...
	ret = i_three_power_on(chip, false);
	if (ret) {
