...
static int ptn5150_poweroff(struct notifier_block *nb, unsigned long event, void *unused);
static struct notifier_block ptn5150_poweroff_notifier = {
        .notifier_call = ptn5150_poweroff,
};
static int ptn5150_poweroff(struct notifier_block *nb, unsigned long event, void *unused)
{
	switch (event) {
		...
		case SYS_POWER_OFF:
			...
			gpio_direction_output(nxp_typec_gpio[CC_EN].gpio,0);
			break;
		default:
			break;
	}
	...
	struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	register_reboot_notifier(&ptn5150_poweroff_notifier);
    info->i2c = client;
...
static int ptn5150_remove(struct i2c_client *client)
{
	...
	unregister_reboot_notifier(&ptn5150_poweroff_notifier);
	kfree(info);
