...
static unsigned int bcm2079x_dev_poll(struct file *filp, poll_table *wait)
{
	...
	poll_wait(filp, &bcm2079x_dev->read_wq, wait);
        ...
        if (bcm2079x_dev->count_irq > 1)
            dev_err(&bcm2079x_dev->client->dev,"dev_poll count_irq=\%d*****\n", bcm2079x_dev->count_irq);

        spin_lock_irqsave(&bcm2079x_dev->irq_enabled_lock, flags);
        gpio_val = gpio_get_value(bcm2079x_dev->irq_gpio);
        dev_info(&bcm2079x_dev->client->dev,"irq_gpio_value %d\n",gpio_val);
        if ((bcm2079x_dev->count_irq > 0) && gpio_val) {
            bcm2079x_dev->count_irq--;
            mask |= POLLIN | POLLRDNORM;
            dev_err(&bcm2079x_dev->client->dev,"dev_poll,mask,count_irq:%d,irq_gpio_val:%d\n",
                                                bcm2079x_dev->count_irq,gpio_val);
        }
        spin_unlock_irqrestore(&bcm2079x_dev->irq_enabled_lock, flags);
				  ...
				  size_t count, loff_t *offset)
{
	...
	**/
	...
	mutex_unlock(&bcm2079x_dev->read_mutex);
        if ((total == 3) &&(count == 3)&&(tmp[0] == 0xff)&&(tmp[1] == 0xff)&&(tmp[2] == 0xff)){
                dev_err(&bcm2079x_dev->client->dev,"not the intended data for host");
                total = 0;
        }
	if (total != count) {
        ...
        dev_info(&bcm2079x_dev->client->dev,"r:%d c:%zu, 0x%02x, 0x%02x, 0x%02x",
