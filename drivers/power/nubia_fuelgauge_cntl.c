...
static int nubia_get_charger_voltage(struct nubia_fg_cntl *batt_cntl)
{
	union power_supply_propval ret = {0,};
...
static int  nubia_update_batt_params(struct nubia_fg_cntl *batt_cntl)
{
	...
	batt_param->chg_vol = nubia_get_charger_voltage(batt_cntl);
...
//!!!To check if it is charging or discharging current <= hrd_ma
static int batt_is_chging_or_little_dischg(struct nubia_fg_params *pbatt, int hrd_ma)
{
    /************************************************************
       * !!!--  Note  --!!!
       *   batt_ma > 0,means it is discharging, batt_ma <= 0,means it is charging
...
static void nubia_batt_worker(struct work_struct *work)
{
	...
	if( pbatt->usb_in 
		&& last_batt_soc==100 
		&& (pbatt->new_soc==99 || pbatt->new_soc==98) ){	
		if( batt_is_chging_or_little_dischg(pbatt,10) ){
			pbatt->new_soc = 100;
	...
	if( pbatt->usb_in 
		&& pbatt->new_batt_status == POWER_SUPPLY_STATUS_FULL
	...
	if(last_batt_soc != 0 && pbatt->new_soc == 0 && !pbatt->usb_in){
		if(pbatt->batt_mv > 3400 && pbatt->batt_mv < 4500){
			pbatt->new_soc = 1;
			batt_low_count = 0;
			pr_info("batt_vol is above 3400,set soc =1 \n");
		}else if(pbatt->batt_mv > 3200 && pbatt->batt_mv < 4500 && batt_low_count < 2){
			batt_low_count ++;
			pbatt->new_soc = 1;
