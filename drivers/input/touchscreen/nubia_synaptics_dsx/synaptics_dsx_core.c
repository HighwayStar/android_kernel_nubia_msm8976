...
static ssize_t synaptics_rmi4_productid_show(struct device *dev,
		struct device_attribute *attr, char *buf);
		...
		struct synaptics_rmi4_fn *fhandler)
{
	...
	if (!bdata->cap_button_map) {
		...
		if (!strncmp(rmi->product_id_string, "NUBIA-JDI", 9) || !strncmp(rmi->product_id_string, "s3320", 5) || !strncmp(rmi->product_id_string, "NX569-JDIA", 10) ) {
