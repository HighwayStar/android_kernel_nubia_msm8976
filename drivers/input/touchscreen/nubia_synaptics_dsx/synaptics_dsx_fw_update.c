...
static int fwu_start_reflash(void)
{
	...
	if (fwu->image == NULL) {
		//note: nx549's panel will no longer update firmware...
		if(!strncmp(rmi->product_id_string, "s3320", 5) || !strncmp(rmi->product_id_string, "NX569-JDIA", 10) ) {
			pr_err("%s:synaptics tp is jdi !\n", __func__);
			retval = secure_memcpy(fwu->image_name, MAX_IMAGE_NAME_LEN,
			FW_IMAGE_NAME_W, sizeof(FW_IMAGE_NAME_W),
			sizeof(FW_IMAGE_NAME_W));
		}
		else if(!strncmp(rmi->product_id_string, "NUBIA-CTCR", 10)) {
			pr_err("%s:synaptics tp is ofilm ctcr!\n", __func__);
			retval = secure_memcpy(fwu->image_name, MAX_IMAGE_NAME_LEN,
				FW_IMAGE_NAME_OCTCR, sizeof(FW_IMAGE_NAME_OCTCR),
				sizeof(FW_IMAGE_NAME_OCTCR));
		}
		else if(!strncmp(rmi->product_id_string, "NUBIA-CTCT", 10)) {
			pr_err("%s:synaptics tp is ofilm ctct!\n", __func__);
			retval = secure_memcpy(fwu->image_name, MAX_IMAGE_NAME_LEN,
				FW_IMAGE_NAME_OCTCT, sizeof(FW_IMAGE_NAME_OCTCT),
				sizeof(FW_IMAGE_NAME_OCTCT));
		}
		else {
			pr_err("%s:synaptics tp unknow product id!\n", __func__);
			retval = -EPERM;
		}
