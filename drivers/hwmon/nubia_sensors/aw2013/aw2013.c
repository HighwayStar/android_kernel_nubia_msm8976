...
static int led_const_on(struct aw2013_control_data *led)
{
	...
	//The value of led->grade_scale_fractions and led->grade_scale_numerator range from 1 to 5.
		...
		if ((led->min_grade == CONST_MIN_GRADE) ||((led->min_grade*led->middle_grade_percentage) < 100) ){
			...
			led_value = led->min_grade*led->middle_grade_percentage/100+led->grade_scale_offset;
			led_value = led_value > 254 ? 254 : led_value;
			write_reg(led,aw2013_regs.pwm0_max_lightness + led->outn, led_value);
			LOG_DEBUG_IF(led->debug_level, "led_const_on,led_value=%d max_current=%d\n", led_value, led->max_current);
		}
	}
	else{
		write_reg(led,aw2013_regs.led0_mode_conf + led->outn, led->side_max_current);
		if ((led->min_grade == CONST_MIN_GRADE) ||((led->min_grade*led->side_grade_percentage) < 100) ){
			write_reg(led,aw2013_regs.pwm0_max_lightness + led->outn, led->min_grade);
		} else {
			led_value = led->min_grade*led->side_grade_percentage/100+led->grade_scale_offset;
			led_value = led_value > 254 ? 254 : led_value;
			//set side light to const value if side_const_value != 0
			if (led->side_const_value > 0)
				led_value = led->side_const_value;
			write_reg(led,aw2013_regs.pwm0_max_lightness + led->outn, led_value);
			LOG_DEBUG_IF(led->debug_level, "led_const_on,led_value=%d side_max_current=%d\n", led_value, led->side_max_current);
		}
	}
...
static int led_auto_breath(struct aw2013_control_data *led) // self breath 0 - 255 version.
{
	if(NULL == led)
		return -EINVAL;
	LOG_DEBUG_IF(led->debug_level, "enter\n");

	if(led->outn == LED_MIDDLE_CHANNEL){
		write_reg(led,aw2013_regs.led0_mode_conf + led->outn, LED_MODE_CONFIG + led->max_current); //FO=FI=MD=1, Imax=10
	} else {
		write_reg(led,aw2013_regs.led0_mode_conf + led->outn, LED_MODE_CONFIG + led->side_max_current); //FO=FI=MD=1, Imax=10
	}
...
static int led_breath_once(struct aw2013_control_data *led)
{
	...
	if ((led->min_grade == CONST_MIN_GRADE) ||((led->min_grade*led->middle_grade_percentage) < 100) ){
		...
		led_value = led->min_grade*led->middle_grade_percentage/100+ led->grade_scale_offset;
		led_value = led_value > 254 ? 254 : led_value;
		write_reg(led,aw2013_regs.pwm0_max_lightness, led_value);
	}
	LOG_DEBUG_IF(led->debug_level, "led_breath_once, led_value=%d .\n", led_value);
	msleep((0x0d << led->Rise_Fall_time)*10 + 100);
	return 0;
...
static ssize_t aw2013_debug_store(struct device *dev,
	struct device_attribute *attr,
	const char *buf, size_t count)
{
	struct aw2013_control_data *led;
...
static ssize_t aw2013_debug_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	struct aw2013_control_data *led;
	struct led_classdev *led_cdev = dev_get_drvdata(dev);
...
static DEVICE_ATTR(debug, 0664, aw2013_debug_show, aw2013_debug_store);
...

{
	...
	ret = of_property_read_u32(client->dev.of_node, "side_max_current",&aw2013_data->side_max_current);
	if (ret < 0) {
		LOG_ERROR("Failure reading side_max_current, rc =  %d\n", ret);
		return -EINVAL;
	}

	ret = of_property_read_u32(client->dev.of_node, "side_const_value",&aw2013_data->side_const_value);
	if (ret < 0) {
		LOG_ERROR("Failure reading side_const_value, rc =  %d\n", ret);
		return -EINVAL;
	}
	...
	while(retry_times--)
	{
		...
		if (ret >= 0) {
			LOG_DEBUG("aw2013_debug: reg[0x02] = %d\n", ret);
			break;
		}
...
static int aw2013_suspend(struct i2c_client *cl, pm_message_t mesg)
{
	...
	LOG_DEBUG_IF(aw2013_data->debug_level, "blink_mode=%d \n", aw2013_data->blink_mode);
	return 0;
};
