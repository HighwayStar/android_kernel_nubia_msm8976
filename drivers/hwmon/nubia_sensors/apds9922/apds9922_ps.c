...
static int apds9922_check_intr(struct apds9922_ps_data *data)
{
	...
	if ( data->ps_detection == 1 ) {
		...
		if(data->ps_data > 254){
			...
			apds9921_dd_set_prx_thresh(client, data->ps_thrd_low+data->ps_offset+data->ps_oil_value, 255);     //假设触碰到油污
		}else{
...
//	apds9921_dd_set_prx_pulses(client, data->ps_pulse);   //会影响ALS吗澹? // 能否使同状态中断
